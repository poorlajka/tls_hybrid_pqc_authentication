/**
 * @file ryde_5s_tcith.h
 * @brief Functions concerning the TCitH part of the RYDE scheme
 */

#ifndef RYDE_5S_TCITH_H
#define RYDE_5S_TCITH_H
...
#define RYDE_5S_BLOCK_LENGTH ((RYDE_5S_VEC_R_MINUS_ONE_BYTES + RYDE_5S_MAT_FQ_BYTES + RYDE_5S_VEC_RHO_BYTES + (RYDE_5S_SECURITY_BYTES - 1)) / RYDE_5S_SECURITY_BYTES)

#if (RYDE_5S_BLOCK_LENGTH > 0xFF)
#error RYDE_5S_BLOCK_LENGTH must fit in uint8_t
#endif

typedef struct {
    rbc_67_vec s[RYDE_5S_PARAM_TAU];
    rbc_67_mat C[RYDE_5S_PARAM_TAU];
    rbc_67_vec v[RYDE_5S_PARAM_TAU];
} ryde_5s_tcith_shares_t;

typedef uint8_t ryde_5s_tcith_alpha_t[RYDE_5S_VEC_RHO_BYTES];
typedef uint8_t ryde_5s_tcith_share_s_t[RYDE_5S_VEC_R_MINUS_ONE_BYTES];
typedef uint8_t ryde_5s_tcith_share_C_t[RYDE_5S_MAT_FQ_BYTES];
typedef size_t ryde_5s_tcith_challenge_t[RYDE_5S_PARAM_TAU];
...
typedef uint8_t ryde_5s_tcith_seed_t[RYDE_5S_SECURITY_BYTES];
typedef uint8_t ryde_5s_tcith_commit_t[RYDE_5S_HASH_BYTES];
...
#define ryde_5s_tcith_shift_to_right(shiftOut, highIn, lowIn, shift, DigitSize)  \
...
void ryde_5s_unpack_matrices_and_vectors(rbc_67_vec *aux_s, rbc_67_mat_fq *aux_C, rbc_67_vec *mid_alpha, const uint8_t *output);
