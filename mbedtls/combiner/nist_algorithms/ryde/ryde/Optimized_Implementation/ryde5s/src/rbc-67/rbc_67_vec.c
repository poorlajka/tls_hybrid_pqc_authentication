/**
 * \file rbc_67_vec.c
 * \brief Implementation of rbc_67_vec.h
 */
...
/**
 * \fn void rbc_67_vec_init(rbc_67_vec* v, uint32_t size)
 * \brief This function initiates a rbc_67_vec element.
 *
 * \param[out] v Pointer to a rbc_67_vec element
 * \param[in] size Size of the vector
 */
void rbc_67_vec_init(rbc_67_vec* v, uint32_t size) {
  *v = calloc(size, sizeof(rbc_67_elt));
...
/**
 * \fn void rbc_67_vec_clear(rbc_67_vec v)
 * \brief This function clears a rbc_67_vec element.
 *
 * \param[out] v rbc_67_vec
 */
...
/**
 * \fn void rbc_67_vec_set_zero(rbc_67_vec v, uint32_t size)
...
/**
 * \fn void rbc_67_vec_set_random(seedexpander_shake_t*, rbc_67_vec v, uint32_t size)
 ...
 * \param[in] size Size of the vector
 */
void rbc_67_vec_set_random(seedexpander_shake_t* ctx, rbc_67_vec v, uint32_t size) {
...
/**
 * \fn void rbc_67_vec_set_random_full_rank_with_one(seedexpander_shake_t* ctx, rbc_67_vec o, uint32_t size) {
...
void rbc_67_vec_set_random_full_rank_with_one(seedexpander_shake_t* ctx, rbc_67_vec o, uint32_t size) {
  int32_t rank_max = RBC_67_FIELD_M < size ? RBC_67_FIELD_M : size;
  int32_t rank = -1;

  while(rank != rank_max) {
    rbc_67_vec_set_random(ctx, o, size - 1);
    rbc_67_elt_set_one(o[size - 1]);
    rank = rbc_67_vec_get_rank(o, size);
  }
}
...
/**
 * \fn void rbc_67_vec_set_random_from_support(seedexpander_shake_t* o, rbc_67_vec o, uint32_t size, const rbc_67_vec support, uint32_t support_size)
...
void rbc_67_vec_set_random_from_support(seedexpander_shake_t* ctx, rbc_67_vec o, uint32_t size, const rbc_67_vec support, uint32_t support_size, uint8_t copy_flag) {
  ...
  if(copy_flag) {
    ...
    while(i != support_size) {
      ...
      if(position < size * (256 / size) && rbc_67_elt_is_zero(o[position % size])) {
        rbc_67_elt_set(o[position % size], support[i]);
        i++;
      }
    ...
    uint32_t l = 0;
    for(i = 0 ; i < size ; ++i) {
      if(rbc_67_elt_is_zero(o[i])) {

        for(j = 0 ; j < support_size ; ++j) {
          if(random2[k] & 0x1) {
            rbc_67_elt_add(o[i], support[j], o[i]);
          }
    ...
    for(i=0 ; i<size ; i++) {
      for(j=0 ; j<support_size ; j++) {
        if(random[k] & 0x01) {
          rbc_67_elt_add(o[i], o[i], support[j]);
        }
...
/**
 * \fn uint32_t rbc_67_vec_gauss(rbc_67_vec v, uint32_t size, rbc_67_vec *other_matrices, uint32_t nMatrices)
...
uint32_t rbc_67_vec_gauss(rbc_67_vec v, uint32_t size, uint8_t reduced_flag, rbc_67_vec *other_matrices, uint32_t nMatrices) {
  ...
  rbc_67_elt tmp, zero;
  uint32_t mask;
  rbc_67_elt_set_zero(zero);

  //For each column
  for(uint32_t p = 0 ; p < size ; p++) {
    rbc_67_elt acc;
    rbc_67_elt_set(acc, v[p]);
    for(uint32_t i=p+1 ; i<size ; i++) {
      for(uint32_t j=0 ; j<RBC_67_ELT_DATA_SIZE ; j++) {
        acc[j] |= v[i][j];
      }
    }

    int column = rbc_67_elt_get_degree(acc);
    column += (column < 0);

    dimension += (!rbc_67_elt_is_zero(acc));

    //For each line below
    for(uint32_t i=p+1 ; i < size ; i++) {
        mask = rbc_67_elt_get_coefficient(v[p], column);
        rbc_67_elt_set_mask1(tmp, zero, v[i], mask);
        rbc_67_elt_add(v[p], v[p], tmp);

        for(uint32_t k=0 ; k<nMatrices ; k++) {
          rbc_67_elt_set_mask1(tmp, zero, other_matrices[k][i], mask);
          rbc_67_elt_add(other_matrices[k][p], other_matrices[k][p], tmp);
        }
    }
    ...
    if(reduced_flag) {
      for(uint32_t i=0 ; i < p ; i++) {
        mask = rbc_67_elt_get_coefficient(v[i], column);
        rbc_67_elt_set_mask1(tmp, v[p], zero, mask);
        rbc_67_elt_add(v[i], v[i], tmp);

        for(uint32_t k=0 ; k<nMatrices ; k++) {
          rbc_67_elt_set_mask1(tmp, other_matrices[k][p], zero, mask);
          rbc_67_elt_add(other_matrices[k][i], other_matrices[k][i], tmp);
        }
      }
    }
...
/**
 * \fn uint32_t rbc_67_vec_get_rank(const rbc_67_vec v, uint32_t size)
...
uint32_t rbc_67_vec_get_rank(const rbc_67_vec v, uint32_t size) {
  ...
  rbc_67_vec_init(&copy, size);
  rbc_67_vec_set(copy, v, size);
  dimension = rbc_67_vec_gauss(copy, size, 0, NULL, 0);
  rbc_67_vec_clear(copy);
...
/**
 * \fn void rbc_67_vec_inner_product(rbc_67_elt o, const rbc_67_vec v1, const rbc_67_vec v2, uint32_t size)
...
void rbc_67_vec_inner_product(rbc_67_elt o, const rbc_67_vec v1, const rbc_67_vec v2, uint32_t size) {
  ...
  for(size_t i = 0 ; i < size ; ++i) {
    rbc_67_elt_mul(t, v1[i], v2[i]);
    rbc_67_elt_add(o, o, t);
  }
  rbc_67_elt_set_zero(t);
}
...
/**
 * \fn void rbc_67_vec_scalar_mul(rbc_67_vec o, const rbc_67_vec v, const rbc_67_elt e, uint32_t size)
 * \brief This functions multiplies a vector of finite field elements by a scalar.
 *
 * \param[out] o rbc_67_vec equal to \f$ e \times v \f$
 * \param[in] v rbc_67_vec
...
/**
 * \fn void rbc_67_vec_to_string(uint8_t* str, const rbc_67_vec v, uint32_t size)
...
void rbc_67_vec_to_string(uint8_t* str, const rbc_67_vec v, uint32_t size) {
  ...
  uint32_t str_size = ((size * RBC_67_FIELD_M) % 8 == 0) ? (size * RBC_67_FIELD_M) / 8 : (size * RBC_67_FIELD_M) / 8 + 1;
  ...
  for(size_t i = 0 ; i < size ; i++) {
    for(size_t j = 1 ; j <= bytes2 ; j++) {
      uint8_t bit = rbc_67_elt_get_coefficient(v[i], RBC_67_FIELD_M - j);
      *(str + index) |= (bit << k % 8);
      k++;
      if(k % 8 == 0) index++;
    }
...
/**
 * \fn void rbc_67_vec_from_string(rbc_67_vec v, uint32_t size, const uint8_t* str)
...
void rbc_67_vec_from_string(rbc_67_vec v, uint32_t size, const uint8_t* str) {
  ...
  for(size_t i = 0 ; i < size ; i++) {
    for(size_t j = 1 ; j <= bytes2 ; j++) {
      ...
      rbc_67_elt_set_coefficient_vartime(v[i], RBC_67_FIELD_M - j, bit);
      k++;
      if(k % 8 == 0) index++;
    }
  }
...
/**
 * \fn void rbc_67_vec_from_bytes(rbc_67_vec o, uint32_t size, const uint8_t *random)
...
void rbc_67_vec_from_bytes(rbc_67_vec o, uint32_t size, uint8_t *random) {
  uint8_t mask = (1 << RBC_67_FIELD_M % 8) - 1;
  if ((RBC_67_FIELD_M % 8) == 0) { mask = 0xff; }

  for(size_t i = 0 ; i < size ; ++i) {
    random[(i + 1) * RBC_67_ELT_UINT8_SIZE - 1] &= mask;
    memcpy(o[i], &random[i * RBC_67_ELT_UINT8_SIZE], RBC_67_ELT_UINT8_SIZE);
  }
}
...
/**
 * \fn void rbc_67_vec_to_bytes( uint8_t *o, const rbc_67_vec v, uint32_t size)
 * \brief This function sets a vector of finite field elements from random bytes.
 *
...
void rbc_67_vec_to_bytes( uint8_t *o, const rbc_67_vec v, uint32_t size) {
  for(size_t i = 0 ; i < size ; ++i) {
    memcpy(&o[i * RBC_67_ELT_UINT8_SIZE], v[i], RBC_67_ELT_UINT8_SIZE);
  }
}
...
/**
 * \fn void rbc_67_vec_print(rbc_67_vec v, uint32_t size)
 * \brief Display an rbc_67_vec element.
 *
 * \param[out] v rbc_67_vec
 * \param[in] size Size of the vector
 */
void rbc_67_vec_print(rbc_67_vec v, uint32_t size) {
  printf("[\n");
  for(size_t i = 0 ; i < size ; ++i) {
    rbc_67_elt_print(v[i]);
    printf("\n");
  }
  printf("]\n");
}
