...
/**
 * \fn int ryde_3s_sign(uint8_t* signature, const uint8_t* message, size_t message_size, const uint8_t* sk)
...
int ryde_3s_sign(uint8_t* signature, const uint8_t* message, size_t message_size, const uint8_t* sk) {
  ...
  memset(signature, 0, RYDE_3S_SIGNATURE_BYTES);
  ...
  uint8_t salt_and_rseed[RYDE_3S_SALT_BYTES + RYDE_3S_SECURITY_BYTES] = {0};
  ...
  rbc_61_vec overline_v, acc_v[RYDE_3S_PARAM_TAU];
  rbc_61_vec overline_s_, acc_s[RYDE_3S_PARAM_TAU];
  rbc_61_mat_fq overline_C, acc_C[RYDE_3S_PARAM_TAU];
  rbc_61_mat overline_D;
  ryde_3s_tcith_shares_t base;
  rbc_61_mat gamma;
  ...
  uint8_t pk[RYDE_3S_PUBLIC_KEY_BYTES] = {0};
  rbc_61_mat H;
  ...
  rbc_61_vec_init(&s, RYDE_3S_PARAM_R);
  rbc_61_mat_fq_init(&C, RYDE_3S_PARAM_R, RYDE_3S_PARAM_N - RYDE_3S_PARAM_R);
  ...
  for(size_t i = 0; i < RYDE_3S_PARAM_TAU; i++ ) {
    rbc_61_vec_init(&(acc_s[i]), RYDE_3S_PARAM_R - 1);
    rbc_61_mat_fq_init(&(acc_C[i]), RYDE_3S_PARAM_R, RYDE_3S_PARAM_N - RYDE_3S_PARAM_R);
    rbc_61_vec_init(&(acc_v[i]), RYDE_3S_PARAM_RHO);
  }

  rbc_61_vec mid_alpha[RYDE_3S_PARAM_TAU];
  for(size_t i = 0; i < RYDE_3S_PARAM_TAU; i++ ) {
    rbc_61_vec_init(&(mid_alpha[i]), RYDE_3S_PARAM_RHO);
  }

  ryde_3s_tcith_shares_init(&base);
  rbc_61_mat_init(&gamma, RYDE_3S_PARAM_N - RYDE_3S_PARAM_K, RYDE_3S_PARAM_RHO);
  rbc_61_vec_init(&base_x, RYDE_3S_PARAM_N - RYDE_3S_PARAM_R);
  rbc_61_vec_init(&base_xL, RYDE_3S_PARAM_N - RYDE_3S_PARAM_R - RYDE_3S_PARAM_K);
  rbc_61_vec_init(&base_xR, RYDE_3S_PARAM_K);
  rbc_61_vec_init(&base_a, RYDE_3S_PARAM_RHO);
  rbc_61_vec_init(&mid_a, RYDE_3S_PARAM_RHO);
  rbc_61_vec_init(&mid_x, RYDE_3S_PARAM_N - 1);
  rbc_61_vec_init(&mid_xL, RYDE_3S_PARAM_N - RYDE_3S_PARAM_K - 1);
  rbc_61_vec_init(&mid_xR, RYDE_3S_PARAM_K);
  ...
  rbc_61_vec_init(&sC, RYDE_3S_PARAM_N - RYDE_3S_PARAM_R);

  // Parse secret key and public key
  ryde_3s_secret_key_from_string(y, H, s, C, sk);
  ryde_3s_public_key_to_string(pk, &sk[RYDE_3S_SECURITY_BYTES], y);

  rbc_61_mat_fq_mul_by_vec_left(sC, (rbc_61_mat_fq)&(C[1]), (rbc_61_vec)&s[1], RYDE_3S_PARAM_R - 1, RYDE_3S_PARAM_N - RYDE_3S_PARAM_R);
  ...
  if (RYDE_randombytes(salt_and_rseed, RYDE_3S_SALT_BYTES + RYDE_3S_SECURITY_BYTES) != EXIT_SUCCESS) {
    memset(salt_and_rseed, 0, RYDE_3S_SALT_BYTES + RYDE_3S_SECURITY_BYTES);
    ...
    ryde_3s_tcith_shares_clear(&base);
  ...
  memcpy(&salt, salt_and_rseed, RYDE_3S_SALT_BYTES);                           // salt
  memcpy(tree[0], &salt_and_rseed[RYDE_3S_SALT_BYTES], RYDE_3S_SECURITY_BYTES);   // root seed
  ryde_3s_ggm_tree_expand(tree, salt);
  ...
  for(size_t e = 0; e < RYDE_3S_PARAM_TAU; e++) {
    ...
    rbc_61_vec_set_zero(base.s[e], RYDE_3S_PARAM_R - 1);
    rbc_61_mat_set_zero(base.C[e], RYDE_3S_PARAM_R, RYDE_3S_PARAM_N - RYDE_3S_PARAM_R);
    rbc_61_vec_set_zero(base.v[e], RYDE_3S_PARAM_RHO);

    rbc_61_vec_set_zero(acc_s[e], RYDE_3S_PARAM_R - 1);
    rbc_61_mat_fq_set_zero(acc_C[e], RYDE_3S_PARAM_R, RYDE_3S_PARAM_N - RYDE_3S_PARAM_R);
    rbc_61_vec_set_zero(acc_v[e], RYDE_3S_PARAM_RHO);
    ...
    for(size_t i = 0; i < N; i++) {
      ...
      rbc_61_vec_add(acc_s[e], acc_s[e], overline_s_, RYDE_3S_PARAM_R - 1);
      rbc_61_mat_fq_add(acc_C[e], acc_C[e], overline_C, RYDE_3S_PARAM_R, RYDE_3S_PARAM_N - RYDE_3S_PARAM_R);
      rbc_61_vec_add(acc_v[e], acc_v[e], overline_v, RYDE_3S_PARAM_RHO);

      // Compute random base
      rbc_61_elt phi_i;
      ryde_3s_tcith_phi(phi_i, i);

      rbc_61_vec_scalar_mul(overline_s_, overline_s_, phi_i, RYDE_3S_PARAM_R - 1);
      rbc_61_mat_fq_mul_by_constant(overline_D, overline_C, phi_i, RYDE_3S_PARAM_R, RYDE_3S_PARAM_N - RYDE_3S_PARAM_R);
      rbc_61_vec_scalar_mul(overline_v, overline_v, phi_i, RYDE_3S_PARAM_RHO);

      rbc_61_vec_add(base.s[e], base.s[e], overline_s_, RYDE_3S_PARAM_R - 1);
      rbc_61_mat_add(base.C[e], base.C[e], overline_D, RYDE_3S_PARAM_R, RYDE_3S_PARAM_N - RYDE_3S_PARAM_R);
      rbc_61_vec_add(base.v[e], base.v[e], overline_v, RYDE_3S_PARAM_RHO);
    }


    // Compute (s - acc_s[e])
    rbc_61_vec_add(acc_s[e], (rbc_61_vec)&s[1], acc_s[e], RYDE_3S_PARAM_R - 1);
    rbc_61_vec_to_string(aux_s_str[e], acc_s[e], RYDE_3S_PARAM_R - 1);

    // Compute (C - acc_C[e])
    rbc_61_mat_fq_add(acc_C[e], C, acc_C[e], RYDE_3S_PARAM_R, RYDE_3S_PARAM_N - RYDE_3S_PARAM_R);
    rbc_61_mat_fq_to_string(aux_C_str[e], acc_C[e], RYDE_3S_PARAM_R, RYDE_3S_PARAM_N - RYDE_3S_PARAM_R);
  ...
  for(size_t e = 0; e < RYDE_3S_PARAM_TAU; e++) {
    ...
    rbc_61_vec_set_zero(base_a, RYDE_3S_PARAM_RHO);
    ...
    rbc_61_vec_set(base_xL, base_x, RYDE_3S_PARAM_N - RYDE_3S_PARAM_R - RYDE_3S_PARAM_K);
    rbc_61_vec_set(base_xR,
                (rbc_61_vec)&(base_x[RYDE_3S_PARAM_N - RYDE_3S_PARAM_R - RYDE_3S_PARAM_K]),
                RYDE_3S_PARAM_K);

    // Calculate base_a
    rbc_61_mat_mul_by_vec_left_transpose(tmp_nk, H, base_xR, RYDE_3S_PARAM_N - RYDE_3S_PARAM_K, RYDE_3S_PARAM_K);
    rbc_61_vec_add((rbc_61_vec)&(tmp_nk[RYDE_3S_PARAM_R]),
                (rbc_61_vec)&(tmp_nk[RYDE_3S_PARAM_R]),
                base_xL,
                RYDE_3S_PARAM_N - RYDE_3S_PARAM_R - RYDE_3S_PARAM_K);
    rbc_61_mat_mul_by_vec_left(base_a, gamma, tmp_nk, RYDE_3S_PARAM_N - RYDE_3S_PARAM_K, RYDE_3S_PARAM_RHO);
    rbc_61_vec_add(base_a, base_a, base.v[e], RYDE_3S_PARAM_RHO);
    rbc_61_vec_to_string(base_a_str[e], base_a, RYDE_3S_PARAM_RHO);

    // Calculate mid_x
    rbc_61_mat_fq_mul_by_vec_left(tmp_nr, (rbc_61_mat_fq)&(C[1]), base.s[e], RYDE_3S_PARAM_R - 1, RYDE_3S_PARAM_N - RYDE_3S_PARAM_R);
    rbc_61_mat_mul_by_vec_left(aux_nr, (rbc_61_mat)&(base.C[e][1]), (rbc_61_vec)&s[1], RYDE_3S_PARAM_R - 1, RYDE_3S_PARAM_N - RYDE_3S_PARAM_R);
    rbc_61_vec_add(tmp_nr, aux_nr, tmp_nr, RYDE_3S_PARAM_N - RYDE_3S_PARAM_R);
    rbc_61_vec_add(tmp_nr, tmp_nr, (rbc_61_vec)(base.C[e][0]), RYDE_3S_PARAM_N - RYDE_3S_PARAM_R);

    rbc_61_vec_set(mid_x, base.s[e], RYDE_3S_PARAM_R - 1);
    rbc_61_vec_set((rbc_61_vec)&mid_x[RYDE_3S_PARAM_R - 1], tmp_nr, RYDE_3S_PARAM_N - RYDE_3S_PARAM_R);

    rbc_61_vec_set(mid_xL, mid_x, RYDE_3S_PARAM_N - RYDE_3S_PARAM_K - 1);
    rbc_61_vec_set(mid_xR, (rbc_61_vec)&(mid_x[RYDE_3S_PARAM_N - RYDE_3S_PARAM_K - 1]), RYDE_3S_PARAM_K);

    // Calculate mid_a
    rbc_61_mat_mul_by_vec_left_transpose(tmp_nk, H, mid_xR, RYDE_3S_PARAM_N - RYDE_3S_PARAM_K, RYDE_3S_PARAM_K);
    rbc_61_vec_add(tmp_nk1, (rbc_61_vec)&(tmp_nk[1]), mid_xL, RYDE_3S_PARAM_N - RYDE_3S_PARAM_K - 1);
    rbc_61_vec_set((rbc_61_vec)&(tmp_nk[1]), tmp_nk1, RYDE_3S_PARAM_N - RYDE_3S_PARAM_K - 1);
    rbc_61_mat_mul_by_vec_left(mid_a, gamma, tmp_nk, RYDE_3S_PARAM_N - RYDE_3S_PARAM_K, RYDE_3S_PARAM_RHO);
    rbc_61_vec_add(mid_a, mid_a, acc_v[e], RYDE_3S_PARAM_RHO);
    rbc_61_vec_to_string(mid_a_str[e], mid_a, RYDE_3S_PARAM_RHO);

    rbc_61_vec_set(mid_alpha[e], mid_a, RYDE_3S_PARAM_RHO);
  }
...
retry:
  ...
  if (ryde_3s_tcith_discard_input_challenge_2(v_grinding)) {
      memset(v_grinding, 0, RYDE_3S_PARAM_W_BYTES);
      ctr += 1;
      goto retry;
  }
  ...
  size_t path_length = ryde_3s_ggm_tree_get_sibling_path(path, (const ryde_3s_ggm_tree_node_t *)tree, psi_i_star);

  if (path_length > RYDE_3S_PARAM_T_OPEN) {
      ctr += 1;
      memset(path, 0, path_length * RYDE_3S_SECURITY_BYTES);
      goto retry;
  }

  // --------------------------------------------------------------------------------------------------------- Signature

  memcpy(&signature[0], salt, RYDE_3S_SALT_BYTES);
  memcpy(&signature[RYDE_3S_SALT_BYTES], &ctr, sizeof(uint64_t));
  memcpy(&signature[RYDE_3S_SALT_BYTES + sizeof(uint64_t)], h2, RYDE_3S_HASH_BYTES);
  memcpy(&signature[RYDE_3S_SALT_BYTES + RYDE_3S_HASH_BYTES + sizeof(uint64_t)], path, RYDE_3S_SECURITY_BYTES * RYDE_3S_PARAM_T_OPEN);

  for(size_t e = 0; e < RYDE_3S_PARAM_TAU; e++) {
    // Commitment concerning the hidden seed
    size_t idx = ryde_3s_tcith_psi(i_star[e], e);
    ryde_3s_tcith_commit(commit, salt, e, i_star[e], seeds[idx]);
    memcpy(&signature[
            RYDE_3S_SALT_BYTES +
            RYDE_3S_HASH_BYTES +
            sizeof(uint64_t) +
  ...
  ryde_3s_pack_matrices_and_vectors(&signature[
                                         RYDE_3S_SALT_BYTES +
                                         RYDE_3S_HASH_BYTES +
                                         sizeof(uint64_t) +
                                         RYDE_3S_PARAM_T_OPEN * RYDE_3S_SECURITY_BYTES +
  ...
  // ------------------------------------------------------------------------------------------------------ Verbose Mode
    ...
    printf("\nsk: "); for(int i = 0 ; i < RYDE_3S_SECRET_KEY_BYTES ; ++i) printf("%02X", sk[i]);
    printf("\npk: "); for(int i = 0 ; i < RYDE_3S_PUBLIC_KEY_BYTES ; ++i) printf("%02X", pk[i]);

    printf("\nx: (s | sC)");
    uint8_t s_string[RYDE_3S_VEC_R_BYTES];
    rbc_61_vec_to_string(s_string, s, RYDE_3S_PARAM_R);
    printf("\n    - s      : "); for(size_t i = 0 ; i < RYDE_3S_VEC_R_BYTES ; i++) { printf("%02X", s_string[i]); }
    memset(s_string, 0, RYDE_3S_VEC_R_BYTES);
    uint8_t C_string[RYDE_3S_MAT_FQ_BYTES] = {0};
    rbc_61_mat_fq_to_string(C_string, C, RYDE_3S_PARAM_R, RYDE_3S_PARAM_N - RYDE_3S_PARAM_R);
    printf("\n    - C      : "); for(size_t i = 0 ; i < RYDE_3S_MAT_FQ_BYTES ; i++) { printf("%02X", C_string[i]); }
    memset(C_string, 0, RYDE_3S_MAT_FQ_BYTES);
    ...
    printf("\n\ntree: ");
    printf("\n    - root   : ");
    for(size_t i = 0 ; i < RYDE_3S_SECURITY_BYTES ; i++) {
      printf("%02X", salt_and_rseed[i + RYDE_3S_SALT_BYTES]);
    }
    ...
    printf("\n\nsigma: "); for(size_t i = 0; i < RYDE_3S_SIGNATURE_BYTES; i++) { printf("%02X", signature[i]); }
  ...
  // -------------------------------------------------------------------------------------------------------- Clear Data
  memset(salt, 0, RYDE_3S_SALT_BYTES);
  memset(salt_and_rseed, 0, RYDE_3S_SALT_BYTES + RYDE_3S_SECURITY_BYTES);
