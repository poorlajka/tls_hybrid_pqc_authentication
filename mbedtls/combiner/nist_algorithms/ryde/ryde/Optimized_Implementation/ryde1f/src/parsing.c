...
/**
 * \fn void ryde_1f_public_key_to_string(uint8_t* pk, const uint8_t* pk_seed, const rbc_53_vec y)
...
void ryde_1f_public_key_to_string(uint8_t* pk, const uint8_t* pk_seed, const rbc_53_vec y) {
  memcpy(pk, pk_seed, RYDE_1F_SECURITY_BYTES);
  rbc_53_vec_to_string(&pk[RYDE_1F_SECURITY_BYTES], y, RYDE_1F_PARAM_N - RYDE_1F_PARAM_K);
}



/**
 * \fn void ryde_1f_public_key_from_string(rbc_53_mat H, rbc_53_vec y, const uint8_t* pk)
...
void ryde_1f_public_key_from_string(rbc_53_mat H, rbc_53_vec y, const uint8_t* pk) {
  ...
  memcpy(pk_seed, pk, RYDE_1F_SECURITY_BYTES);
  ...
  // Compute syndrome
  rbc_53_vec_from_string(y, RYDE_1F_PARAM_N - RYDE_1F_PARAM_K, &pk[RYDE_1F_SECURITY_BYTES]);
...
/**
 * \fn void ryde_1f_secret_key_to_string(uint8_t* sk, const uint8_t* seed, const uint8_t* pk)
 ...
 * \param[in] pk String containing the public key
 */
void ryde_1f_secret_key_to_string(uint8_t* sk, const uint8_t* sk_seed, const uint8_t* pk) {
  memcpy(sk, sk_seed, RYDE_1F_SECURITY_BYTES);
  memcpy(&sk[RYDE_1F_SECURITY_BYTES], pk, RYDE_1F_SECURITY_BYTES);
}



/**
* \fn void ryde_1f_secret_key_from_string(rbc_53_vec y, rbc_53_mat H, rbc_53_vec s, rbc_53_mat_fq C, const uint8_t* sk)
...
void ryde_1f_secret_key_from_string(rbc_53_vec y, rbc_53_mat H, rbc_53_vec s, rbc_53_mat_fq C, const uint8_t* sk) {
  ...
  memcpy(sk_seed, sk, RYDE_1F_SECURITY_BYTES);
  ...
  // Compute public key
  memcpy(pk_seed, &sk[RYDE_1F_SECURITY_BYTES], RYDE_1F_SECURITY_BYTES);
  seedexpander_shake_init(&pk_seedexpander, pk_seed, RYDE_1F_SECURITY_BYTES, NULL, 0);
