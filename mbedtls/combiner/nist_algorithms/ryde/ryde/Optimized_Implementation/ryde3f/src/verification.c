...
/**
 * \fn int ryde_3f_verify(const uint8_t* message, size_t message_size, const uint8_t* signature, size_t signature_size, const uint8_t* pk)
...
int ryde_3f_verify(const uint8_t* signature, size_t signature_size, const uint8_t* message, size_t message_size, const uint8_t* pk) {
  ...
  if(signature_size != RYDE_3F_SIGNATURE_BYTES) { return EXIT_FAILURE; }
  ...
  uint8_t h2_[RYDE_3F_HASH_BYTES] = {0};
  ...
  ryde_3f_tcith_shares_init(&shares);
  rbc_61_mat_init(&gamma, RYDE_3F_PARAM_N - RYDE_3F_PARAM_K, RYDE_3F_PARAM_RHO);
  rbc_61_vec_init(&share_x, RYDE_3F_PARAM_N - 1);
  rbc_61_vec_init(&share_xL, RYDE_3F_PARAM_N - RYDE_3F_PARAM_K - 1);
  rbc_61_vec_init(&share_xR, RYDE_3F_PARAM_K);
  rbc_61_vec_init(&mid_a, RYDE_3F_PARAM_RHO);
  rbc_61_vec_init(&base_a, RYDE_3F_PARAM_RHO);
  rbc_61_vec_init(&share_a, RYDE_3F_PARAM_RHO);
  rbc_61_vec_init(&aux_nk, RYDE_3F_PARAM_N - RYDE_3F_PARAM_K);
  ...
  rbc_61_vec_init(&tmp_r1, RYDE_3F_PARAM_R - 1);


  rbc_61_vec aux_s[RYDE_3F_PARAM_TAU], mid_alpha[RYDE_3F_PARAM_TAU];
  rbc_61_mat_fq aux_C[RYDE_3F_PARAM_TAU];
  for(size_t i = 0; i < RYDE_3F_PARAM_TAU; i++ ) {
    rbc_61_vec_init(&(aux_s[i]), RYDE_3F_PARAM_R - 1);
    rbc_61_mat_fq_init(&(aux_C[i]), RYDE_3F_PARAM_R, RYDE_3F_PARAM_N - RYDE_3F_PARAM_R);
    rbc_61_vec_init(&(mid_alpha[i]), RYDE_3F_PARAM_RHO);
  }
  ryde_3f_unpack_matrices_and_vectors(aux_s, aux_C, mid_alpha,
                                   &signature[
                                           RYDE_3F_SALT_BYTES +
                                           RYDE_3F_HASH_BYTES +
                                           sizeof(uint64_t) +
  ...
  // Parse public key
  ryde_3f_public_key_from_string(H, y, pk);
  ...
  memcpy(salt, &signature[0], RYDE_3F_SALT_BYTES);
  memcpy(&ctr, &signature[RYDE_3F_SALT_BYTES], sizeof(uint64_t));
  memcpy(h2,   &signature[RYDE_3F_SALT_BYTES + sizeof(uint64_t)], RYDE_3F_HASH_BYTES);
  memcpy(path, &signature[RYDE_3F_SALT_BYTES + RYDE_3F_HASH_BYTES + sizeof(uint64_t)], RYDE_3F_SECURITY_BYTES * RYDE_3F_PARAM_T_OPEN);
  ...
  for(size_t i = 0; i < RYDE_3F_PARAM_T_OPEN; i++) {
      uint8_t zero[RYDE_3F_SECURITY_BYTES] = {0};
      if (memcmp(zero, &path[i], RYDE_3F_SECURITY_BYTES) == 0) { continue; }
      path_length += 1;
  }
  ...
  int wrong_sibling_path = ryde_3f_ggm_tree_partial_expand(tree, salt, (const ryde_3f_ggm_tree_node_t*)path, path_length, psi_i_star);
  ryde_3f_ggm_tree_get_leaves(seeds, tree);


  for(size_t e = 0; e < RYDE_3F_PARAM_TAU; e++) {
    // Set to zero the accumulator vector and matrix
    rbc_61_vec_set_zero(shares.s[e], RYDE_3F_PARAM_R - 1);
    rbc_61_mat_set_zero(shares.C[e], RYDE_3F_PARAM_R, RYDE_3F_PARAM_N - RYDE_3F_PARAM_R);
    rbc_61_vec_set_zero(shares.v[e], RYDE_3F_PARAM_RHO);

    rbc_61_elt phi_i_star;
    ryde_3f_tcith_phi(phi_i_star, i_star[e]);
    ...
    for(size_t i = 0; i < N; i++) {
      ...
      if (i == (size_t)i_star[e]) {
        memcpy(commit,
               &signature[
                  RYDE_3F_SALT_BYTES + RYDE_3F_HASH_BYTES + sizeof(uint64_t) + RYDE_3F_PARAM_T_OPEN * RYDE_3F_SECURITY_BYTES +
        ...
        // Compute shares
        rbc_61_elt scalar;
        ryde_3f_tcith_phi(scalar, i);
        rbc_61_elt_add(scalar, phi_i_star, scalar);

        rbc_61_vec_scalar_mul(overline_s_, overline_s_, scalar, RYDE_3F_PARAM_R - 1);
        rbc_61_mat_fq_mul_by_constant(overline_D, overline_C, scalar, RYDE_3F_PARAM_R, RYDE_3F_PARAM_N - RYDE_3F_PARAM_R);
        rbc_61_vec_scalar_mul(overline_v, overline_v, scalar, RYDE_3F_PARAM_RHO);

        rbc_61_vec_add(shares.s[e], shares.s[e], overline_s_, RYDE_3F_PARAM_R - 1);
        rbc_61_mat_add(shares.C[e], shares.C[e], overline_D, RYDE_3F_PARAM_R, RYDE_3F_PARAM_N - RYDE_3F_PARAM_R);
        rbc_61_vec_add(shares.v[e], shares.v[e], overline_v, RYDE_3F_PARAM_RHO);
      }
    }


    // Operations concerning vector se
    rbc_61_vec_to_string(aux_s_str[e], aux_s[e], RYDE_3F_PARAM_R - 1);
    rbc_61_vec_scalar_mul(overline_s_, aux_s[e], phi_i_star, RYDE_3F_PARAM_R - 1);

    // Operations concerning matrix Ce
    rbc_61_mat_fq_to_string(aux_C_str[e], aux_C[e], RYDE_3F_PARAM_R, RYDE_3F_PARAM_N - RYDE_3F_PARAM_R);
    rbc_61_mat_fq_mul_by_constant(overline_D, aux_C[e], phi_i_star, RYDE_3F_PARAM_R, RYDE_3F_PARAM_N - RYDE_3F_PARAM_R);
  ...
  for(size_t e = 0; e < RYDE_3F_PARAM_TAU; e++) {
    ...
    rbc_61_vec_set_zero(tmp_r1, RYDE_3F_PARAM_R - 1);
    rbc_61_vec_set_zero(aux_nk, RYDE_3F_PARAM_N - RYDE_3F_PARAM_K);
    rbc_61_vec_set_zero(aux_nr, RYDE_3F_PARAM_N - RYDE_3F_PARAM_R);
    rbc_61_vec_set_zero(share_a, RYDE_3F_PARAM_RHO);
    ...
    rbc_61_vec_scalar_mul(tmp_r1, shares.s[e], phi_i_star, RYDE_3F_PARAM_R - 1);
    rbc_61_mat_mul_by_vec_left(tmp_nr,
                            (rbc_61_mat)&(shares.C[e][1]),
                            shares.s[e],
                            RYDE_3F_PARAM_R - 1,
                            RYDE_3F_PARAM_N - RYDE_3F_PARAM_R);
    rbc_61_vec_scalar_mul(aux_nr, (rbc_61_vec)shares.C[e][0], phi_i_star, RYDE_3F_PARAM_N - RYDE_3F_PARAM_R);
    rbc_61_vec_add(tmp_nr, aux_nr, tmp_nr, RYDE_3F_PARAM_N - RYDE_3F_PARAM_R);

    rbc_61_vec_set(share_x, tmp_r1, RYDE_3F_PARAM_R - 1);
    rbc_61_vec_set((rbc_61_vec)&share_x[RYDE_3F_PARAM_R - 1], tmp_nr, RYDE_3F_PARAM_N - RYDE_3F_PARAM_R);

    rbc_61_vec_set(share_xL, share_x, RYDE_3F_PARAM_N - RYDE_3F_PARAM_K - 1);
    rbc_61_vec_set(share_xR, (rbc_61_vec)&(share_x[RYDE_3F_PARAM_N - RYDE_3F_PARAM_K - 1]), RYDE_3F_PARAM_K);

    // Calculate share_a
    rbc_61_mat_mul_by_vec_left_transpose(tmp_nk, H, share_xR, RYDE_3F_PARAM_N - RYDE_3F_PARAM_K, RYDE_3F_PARAM_K);
    rbc_61_elt_add(tmp_nk[0], tmp_nk[0], phi_i_star_squared);
    rbc_61_vec_add(tmp_nk1, (rbc_61_vec)&(tmp_nk[1]), share_xL, RYDE_3F_PARAM_N - RYDE_3F_PARAM_K - 1);
    rbc_61_vec_set((rbc_61_vec)&(tmp_nk[1]), tmp_nk1, RYDE_3F_PARAM_N - RYDE_3F_PARAM_K - 1);
    rbc_61_vec_scalar_mul(aux_nk, y, phi_i_star_squared, RYDE_3F_PARAM_N - RYDE_3F_PARAM_K);
    rbc_61_vec_add(tmp_nk, tmp_nk, aux_nk, RYDE_3F_PARAM_N - RYDE_3F_PARAM_K);
    rbc_61_mat_mul_by_vec_left(share_a, gamma, tmp_nk, RYDE_3F_PARAM_N - RYDE_3F_PARAM_K, RYDE_3F_PARAM_RHO);
    rbc_61_vec_add(share_a, share_a, shares.v[e], RYDE_3F_PARAM_RHO);

    // Calculate base_a
    rbc_61_vec_to_string(mid_a_str[e], mid_alpha[e], RYDE_3F_PARAM_RHO);
    rbc_61_vec_scalar_mul(mid_a, mid_alpha[e], phi_i_star, RYDE_3F_PARAM_RHO);
    rbc_61_vec_add(base_a, share_a, mid_a, RYDE_3F_PARAM_RHO);
  ...
  // ------------------------------------------------------------------------------------------------------ Verbose Mode
    ...
    printf("\n\nsigma: "); for(size_t i = 0; i < RYDE_3F_SIGNATURE_BYTES; i++) { printf("%02X", signature[i]); } printf("\n");
    ...
    printf("\n\nh2\': "); for(size_t i = 0 ; i < RYDE_3F_HASH_BYTES ; i++) { printf("%02X", h2_[i]); }
    printf("\n");
  #endif
  ...
  ryde_3f_tcith_shares_clear(&shares);
  ...
  // ------------------------------------------------------------------------------------------------------ Verification
  if ((memcmp(h2, h2_, RYDE_3F_HASH_BYTES) != 0) || wrong_sibling_path || ryde_3f_tcith_discard_input_challenge_2(v_grinding)) {
    memset(h1, 0, RYDE_3F_HASH_BYTES);
    memset(h2, 0, RYDE_3F_HASH_BYTES);;
    memset(h2_, 0, RYDE_3F_HASH_BYTES);
