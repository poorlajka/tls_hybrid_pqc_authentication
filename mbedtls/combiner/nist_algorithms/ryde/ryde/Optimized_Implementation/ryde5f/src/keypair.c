...
/**
 * \fn int ryde_5f_keygen(uint8_t* pk, uint8_t* sk)
 ...
 */
int ryde_5f_keygen(uint8_t* pk, uint8_t* sk) {
  ...
  if (RYDE_randombytes(sk_seed, RYDE_5F_SECURITY_BYTES) != EXIT_SUCCESS) {
    memset(sk_seed, 0, RYDE_5F_SECURITY_BYTES);
    return EXIT_FAILURE;
  }
  if (RYDE_randombytes(pk_seed, RYDE_5F_SECURITY_BYTES) != EXIT_SUCCESS) {
    memset(sk_seed, 0, RYDE_5F_SECURITY_BYTES);
    memset(pk_seed, 0, RYDE_5F_SECURITY_BYTES);
    return EXIT_FAILURE;
  }
  ...
  ryde_5f_public_key_to_string(pk, pk_seed, y);
  ryde_5f_secret_key_to_string(sk, sk_seed, pk_seed);

  #ifdef VERBOSE
    printf("\n\n\n### KEYGEN ###");

    printf("\n\nsk_seed: "); for(int i = 0 ; i < RYDE_5F_SECURITY_BYTES ; ++i) printf("%02X", sk_seed[i]);
    printf("\npk_seed: ");   for(int i = 0 ; i < RYDE_5F_SECURITY_BYTES ; ++i) printf("%02X", pk_seed[i]);
