...
int main(void) {
    ...
    unsigned long long ryde_5f_keypair_mean = 0, ryde_5f_RYDE_crypto_sign_mean = 0, ryde_5f_RYDE_crypto_sign_open_mean = 0;
...
//    (void)syscall(SYS_getrandom, seed, 48, 0);
    ...
    for(size_t i = 0 ; i < NB_TEST ; i++) {
        RYDE_crypto_sign_keypair(ryde_5f_pk, ryde_5f_sk);
    }
    ...
    for(size_t i = 0 ; i < NB_SAMPLES ; i++) {
        ...
        for(size_t j = 0 ; j < NB_TEST ; j++) {
            ...
            t1 = cpucyclesStart();
            RYDE_crypto_sign(ryde_5f_sm, &ryde_5f_smlen, ryde_5f_m, ryde_5f_mlen, ryde_5f_sk);
            t2 = cpucyclesStop();

            timer += t2 - t1;
        }

        ryde_5f_RYDE_crypto_sign_mean += timer / NB_TEST;
    }
    printf("Benchmark (RYDE_crypto_sign)\n");
    ...
    for(size_t i = 0 ; i < NB_SAMPLES ; i++) {
        ...
        RYDE_crypto_sign_keypair(ryde_5f_pk, ryde_5f_sk);
        RYDE_crypto_sign(ryde_5f_sm, &ryde_5f_smlen, ryde_5f_m, ryde_5f_mlen, ryde_5f_sk);
        if (RYDE_crypto_sign_open(ryde_5f_m, &ryde_5f_mlen, ryde_5f_sm, ryde_5f_smlen, ryde_5f_pk) == -1) { ryde_5f_failures++; }
        timer = 0;
        ...
        for(size_t j = 0 ; j < NB_TEST ; j++) {
            ...
            t1 = cpucyclesStart();
            RYDE_crypto_sign_open(ryde_5f_m, &ryde_5f_mlen, ryde_5f_sm, ryde_5f_smlen, ryde_5f_pk);
            t2 = cpucyclesStop();

            timer += t2 - t1;
        }

        ryde_5f_RYDE_crypto_sign_open_mean += timer / NB_TEST;
    }
    printf("Benchmark (RYDE_crypto_sign_open)\n");
    ...
    printf("\n  RYDE_crypto_sign_keypair: %lld CPU cycles", ryde_5f_keypair_mean / NB_SAMPLES);
    printf("\n  RYDE_crypto_sign:         %lld CPU cycles", ryde_5f_RYDE_crypto_sign_mean / NB_SAMPLES);
    printf("\n  RYDE_crypto_sign_open:    %lld CPU cycles", ryde_5f_RYDE_crypto_sign_open_mean / NB_SAMPLES);
    printf("\n\n");
