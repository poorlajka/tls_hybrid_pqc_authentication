...
/**
 * \fn int ryde_1s_keygen(uint8_t* pk, uint8_t* sk)
 ...
 */
int ryde_1s_keygen(uint8_t* pk, uint8_t* sk) {
  ...
  if (RYDE_randombytes(sk_seed, RYDE_1S_SECURITY_BYTES) != EXIT_SUCCESS) {
    memset(sk_seed, 0, RYDE_1S_SECURITY_BYTES);
    return EXIT_FAILURE;
  }
  if (RYDE_randombytes(pk_seed, RYDE_1S_SECURITY_BYTES) != EXIT_SUCCESS) {
    memset(sk_seed, 0, RYDE_1S_SECURITY_BYTES);
    memset(pk_seed, 0, RYDE_1S_SECURITY_BYTES);
    return EXIT_FAILURE;
  }
  ...
  ryde_1s_public_key_to_string(pk, pk_seed, y);
  ryde_1s_secret_key_to_string(sk, sk_seed, pk_seed);

  #ifdef VERBOSE
    printf("\n\n\n### KEYGEN ###");

    printf("\n\nsk_seed: "); for(int i = 0 ; i < RYDE_1S_SECURITY_BYTES ; ++i) printf("%02X", sk_seed[i]);
    printf("\npk_seed: ");   for(int i = 0 ; i < RYDE_1S_SECURITY_BYTES ; ++i) printf("%02X", pk_seed[i]);
