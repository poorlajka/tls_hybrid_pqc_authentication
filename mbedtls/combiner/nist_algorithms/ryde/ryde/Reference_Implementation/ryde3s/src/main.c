...
int main(void) {
    ...
    unsigned long long ryde_3s_mlen = 22;
    unsigned long long ryde_3s_smlen;

    // Message corresponds with: Rank sYndrome DEcoding
    unsigned char ryde_3s_m[] = {0x52, 0x61, 0x6e, 0x6b, 0x20, 0x73, 0x59, 0x6e, 0x64, 0x72, 0x6f, 0x6d,
...
//    (void)syscall(SYS_getrandom, seed, 48, 0);
    ...
    t1 = cpucyclesStart();
    if (RYDE_crypto_sign_keypair(ryde_3s_pk, ryde_3s_sk) == -1) {
        printf("\nnFailed\n\n");
        return -1;
    }
    t2 = cpucyclesStop();
    ...
    t3 = cpucyclesStart();
    if (RYDE_crypto_sign(ryde_3s_sm, &ryde_3s_smlen, ryde_3s_m, ryde_3s_mlen, ryde_3s_sk) != 0) {
        printf("\nnFailed\n\n");
        return -1;
    };
    t4 = cpucyclesStart();
    ...
    t5 = cpucyclesStart();
    if (RYDE_crypto_sign_open(ryde_3s_m, &ryde_3s_mlen, ryde_3s_sm, ryde_3s_smlen, ryde_3s_pk) == -1) {
        printf("\nnFailed\n\n");
        return -1;
    }
    t6 = cpucyclesStart();
    ...
    printf("\n\n");
    printf("\n sk: "); for(int k = 0 ; k < CRYPTO_SECRETKEYBYTES ; ++k) printf("%02x", ryde_3s_sk[k]);
    printf("\n pk: "); for(int k = 0 ; k < CRYPTO_PUBLICKEYBYTES ; ++k) printf("%02x", ryde_3s_pk[k]);
    printf("\n  m: "); for(int k = 0 ; k < (int)ryde_3s_mlen ; ++k) printf("%02x", ryde_3s_m[k]);
    printf("\n sm: "); for(int k = 0 ; k < ((int)ryde_3s_smlen) ; ++k) printf("%02x", ryde_3s_sm[k]);
    #endif
