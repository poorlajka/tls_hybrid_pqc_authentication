...
int main(void) {
    ...
    for(int i=0; i<1; i++) {
        ...
        ret = MQOM_crypto_sign_keypair(pk, sk);
        if(ret) {
            printf("Failure (num %d): MQOM_crypto_sign_keypair\n", i);
            continue;
        }
        ...
        ret = MQOM_crypto_sign(sm, &smlen, m, MLEN, sk);
        if(ret) {
            printf("Failure (num %d): MQOM_crypto_sign\n", i);
            continue;
        }

        // Verify/Open the signature
        ret = MQOM_crypto_sign_open(m2, &m2len, sm, smlen, pk);
        if(ret) {
            printf("Failure (num %d): MQOM_crypto_sign_open\n", i);
            continue;
        }
