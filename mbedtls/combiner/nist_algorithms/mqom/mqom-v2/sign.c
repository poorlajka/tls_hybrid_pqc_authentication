...
int MQOM_SampleChallenge(uint8_t hash[MQOM2_PARAM_DIGEST_SIZE], uint16_t i_star[MQOM2_PARAM_TAU], uint8_t nonce[4])
{
    int ret = -1;
    int e;
	xof_context xof_ctx;
...
int Sign(const uint8_t sk[MQOM2_SK_SIZE], const uint8_t *msg, unsigned long long mlen, const uint8_t salt[MQOM2_PARAM_SALT_SIZE], const uint8_t mseed[MQOM2_PARAM_SEED_SIZE], uint8_t sig[MQOM2_SIG_SIZE]) {
    ...
    ret = MQOM_SampleChallenge(hash, i_star, nonce);
...
int MQOM_crypto_sign_signature(uint8_t *sig,
        ...
        const unsigned char *sk)
{
    ...
    ret = MQOM_crypto_sign_signature(sm+mlen, smlen, message, mlen, sk); ERR(ret, err);
    if(smlen != NULL) {
        *smlen += mlen;
    }
...
int MQOM_crypto_sign_verify(const uint8_t *sig,
                       unsigned long long siglen,
                       const uint8_t *m,
                       unsigned long long mlen,
                       const uint8_t *pk)
...
int MQOM_crypto_sign_open(
    ...
    *mlen = smlen-(unsigned long long) MQOM2_SIG_SIZE;
    ...
    int ret = MQOM_crypto_sign_verify(sm+(*mlen), MQOM2_SIG_SIZE, message, *mlen, pk);
    if (ret) {
        return ret;
    }
