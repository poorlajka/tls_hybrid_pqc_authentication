...
int main(void) {
    ...
    do {
        ...
        if ( (ret_val = MQOM_crypto_sign_keypair(pk2, sk2)) != 0) {
            printf("MQOM_crypto_sign_keypair returned <%d>\n", ret_val);
            return KAT_CRYPTO_FAILURE;
        }
        if ( memcmp(pk, pk2, CRYPTO_PUBLICKEYBYTES)) {
            printf("MQOM_crypto_sign_keypair returned bad 'pk' value\n");
            return KAT_CRYPTO_FAILURE;
        }
        if ( memcmp(sk, sk2, CRYPTO_SECRETKEYBYTES)) {
            printf("MQOM_crypto_sign_keypair returned bad 'sk' value\n");
            return KAT_CRYPTO_FAILURE;
        }
        
        if ( (ret_val = MQOM_crypto_sign(sm2, &smlen2, m, mlen, sk2)) != 0) {
            printf("MQOM_crypto_sign returned <%d>\n", ret_val);
            return KAT_CRYPTO_FAILURE;
        }
        if( smlen != smlen2 ) {
            printf("MQOM_crypto_sign returned bad 'smlen': Got <%llu>, expected <%llu>\n", smlen2, smlen);
            return KAT_CRYPTO_FAILURE;
        }
        if ( memcmp(sm, sm2, smlen) ) {
            printf("MQOM_crypto_sign returned bad 'sm' value\n");
            return KAT_CRYPTO_FAILURE;
        }
        
        if ( (ret_val = MQOM_crypto_sign_open(m1, &mlen1, sm, smlen, pk)) != 0) {
            printf("MQOM_crypto_sign_open returned <%d>\n", ret_val);
            return KAT_CRYPTO_FAILURE;
        }
        
        if ( mlen != mlen1 ) {
            printf("MQOM_crypto_sign_open returned bad 'mlen': Got <%llu>, expected <%llu>\n", mlen1, mlen);
            return KAT_CRYPTO_FAILURE;
        }
        
        if ( memcmp(m, m1, mlen) ) {
            printf("MQOM_crypto_sign_open returned bad 'm' value\n");
            return KAT_CRYPTO_FAILURE;
        }
