                          ...
                          const uint8_t *sk)
{
    ...
    int ret = sdith_sign(sig, &siglen2, m, mlen, sk, salt, seed);
    *siglen = (unsigned long long) siglen2;
    return ret;
}

int SDITH_crypto_sign(
...
int crypto_sign_SDITH_verify(const uint8_t *sig,
                       unsigned long long siglen,
                       const uint8_t *m,
                       unsigned long long mlen,
                       const uint8_t *pk)
{
    return sdith_sign_SDITH_verify(sig, siglen, m, mlen, pk);
}

int SDITH_crypto_sign_open(
    ...
    int ret = crypto_sign_SDITH_verify(sig, signature_len, message, message_len, pk);
    if (ret) {
        return ret;
    }
