...
int sdith_threshold_nfpr_sign(uint8_t* sig, size_t* siglen,
    ...
    for(e=0; e<PARAM_NB_EXECUTIONS; e++) {
        ...
        for(i=0; i<PARAM_NB_PARTIES; i++) {
            compute_complete_share(share, plain, rnd[e], (uint8_t) i);
            commit_share(share_commitments[i], share, ssig->salt, (uint16_t) e, (uint16_t) i);
        }
...
int sdith_threshold_nfpr_sign_SDITH_verify(const uint8_t* sig, size_t siglen,
                const uint8_t* m, size_t mlen,
                const uint8_t* pk
                ) {
    uint32_t i, e, p;
