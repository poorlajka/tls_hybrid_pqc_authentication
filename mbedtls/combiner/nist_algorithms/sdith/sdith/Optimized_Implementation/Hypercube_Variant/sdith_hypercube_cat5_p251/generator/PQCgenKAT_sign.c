...
main()
{
    ...
    SDITH_randombytes_init(entropy_input, NULL, 256);
    for (int i=0; i<100; i++) {
        fprintf(fp_req, "count = %d\n", i);
        SDITH_randombytes(seed, 48);
        fprintBstr(fp_req, "seed = ", seed, 48);
        mlen = 33*(i+1);
        fprintf(fp_req, "mlen = %llu\n", mlen);
        SDITH_randombytes(msg, mlen);
    ...
    do {
        ...
        SDITH_randombytes_init(seed, NULL, 256);
        ...
        if ( (ret_val = SDITH_crypto_sign_keypair(pk, sk)) != 0) {
        ...
        if ( (ret_val = SDITH_crypto_sign(sm, &smlen, m, mlen, sk)) != 0) {
        ...
        if ( (ret_val = SDITH_crypto_sign_open(m1, &mlen1, sm, smlen, pk)) != 0) {
