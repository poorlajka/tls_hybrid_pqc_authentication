...
int SDITH_randombytes(unsigned char *x, unsigned long long xlen);
...
void remove_one_degree_factor_from_monic(fpoly_t* out, const fpoly_t* in, fpoly_t in_degree, fpoly_t alpha) {
    ...
    for(int i=in_degree-2; i>=0; i--)
        out[i] = fpoints_add(in[i+1], p251_mul_naive(alpha, out[i+1]));
}
...
isd_instance_t generate_inhomogeneous_sd_instance(sdith_compressed_key_t const* sk) {
  ...
  uint8_t* random_tape = (uint8_t*)aligned_alloc(32, 4096);
  memset(random_tape, 0, 4096);
  sdith_xof_next_bytes_mod251(rng_ctx, random_tape, 4096);

  uint8_t* random_tape_256 = (uint8_t*)aligned_alloc(32, 4096);
  memset(random_tape_256, 0, 4096);
  sdith_xof_next_bytes(rng_ctx, random_tape_256, 4096);
  ...
  uint8_t* random_tape_256_ptr = random_tape_256;
  ...
  for (uint64_t i_d = 0; i_d < PARAM_d; ++i_d) {
    ...
    while (i < PAR_wd) {
      ...
      non_zero_pos[i] = *(random_tape_256_ptr++);
    ...
    for (uint64_t i = 0; i < PAR_wd; ++i) {
      fpoly_t minus_pos = fpoints_sub(0, non_zero_pos[i]);
      for (uint64_t j = i + 1; j >= 1; --j) {
        q[j] = fpoints_add(q[j-1], p251_mul_naive(q[j], minus_pos));
      }
      q[0] = p251_mul_naive(q[0], minus_pos);
    }
    ...
    memset(p, 0, sizeof(p));
    for (uint64_t i = 0; i < PAR_md; ++i) {
      fpoly_t scalar = p251_mul_naive(leading_coefficients_of_lj_for_S[i], x[i_d][i]);

      fpoly_t temp[PAR_md];
      memset(temp, 0, sizeof(temp));
      remove_one_degree_factor_from_monic(temp, f_poly, PAR_md, i);
      for (uint64_t j = 0; j < PAR_md; j++) {
        s[i_d][j] = fpoints_add(s[i_d][j], p251_mul_naive(temp[j], scalar));
      }

      remove_one_degree_factor_from_monic(temp, q, PAR_wd, i);
      for (uint64_t j = 0; j < PAR_wd; j++) {
        p[j] = fpoints_add(p[j], p251_mul_naive(temp[j], scalar));
      }
    }
  ...
  // Sample H_a
  seed_t H_a_seed;
  memcpy(H_a_seed, random_tape_256_ptr, PARAM_seed_size);
  random_tape_256_ptr += PARAM_seed_size;
  XOF_CTX *H_a_rng_ctx = sdith_rng_create_xof_ctx(H_a_seed, PARAM_seed_size);
  fsd_t H_a[PARAM_k][PAR_y_size];
  sdith_xof_next_bytes_mod251(H_a_rng_ctx, H_a, PARAM_k*PAR_y_size);
  ...
  {
    fpoly_t s_A_inv[PARAM_k];
    for (uint64_t i = 0; i < PARAM_k; ++i) {
      s_A_inv[i] = p251_reduce_16(251 - reps.s_A[i]);
    }

    memset(y, 0, sizeof(y));
    memcpy(y, s_B, PAR_y_size);
    for (uint64_t slice = 0; slice < PAR_ha_nslice; ++slice) {
      p251_vec_mat128cols_muladd_ct(y[slice], s_A_inv, reps.H_a[slice], PARAM_k);
    }
  ...
  for (uint64_t k=0; k<PARAM_k; ++k) {
    for (uint64_t j = 0; j < PAR_y_size; ++j) {
      s_raw[PARAM_k + j] = fpoints_add(s_raw[PARAM_k + j], p251_mul_naive(H_a[k][j], reps.s_A[k]));
    }
  }
  ...
  for (uint64_t i_d=0; i_d<PARAM_d; ++i_d) {
    ...
    for (uint64_t k=0; k<PAR_md; ++k) {
      ...
      for (uint64_t j=PAR_md; j>=1; --j) {
        c = fpoints_add(p251_mul_naive(c, k), recomputed_s[i_d][j-1]);
      }
      reconstructed_x[k] = c;
    }
    int64_t weight = 0;
    ...
    for (uint64_t k=0; k<PAR_md; ++k) {
      ...
      for (uint64_t j=PAR_wd; j>=1; --j) {
        c = fpoints_add(p251_mul_naive(c, k), reps.q_poly[i_d][j-1]);
      }
      REQUIRE_DRAMATICALLY(c == 0, "Q poly didn't cancel out on non-zero x[%ld]: %d", k, c);
    }

    // TODO(stevenyue): verify SQ-PF=0
    fpoints_t r = 3773370153;
    fpoints_t sr = 0;
    for (uint64_t j=PAR_md; j>=1; --j) {
      sr = fpoints_add(fpoints_mul(sr, r), recomputed_s[i_d][j-1]);
    }
    fpoints_t qr = 1;
    for (uint64_t j=PAR_wd; j>=1; --j) {
      qr = fpoints_add(fpoints_mul(qr, r), reps.q_poly[i_d][j-1]);
    }
    fpoints_t pr = 0;
    for (uint64_t j=PAR_wd; j>=1; --j) {
      pr = fpoints_add(fpoints_mul(pr, r), reps.p_poly[i_d][j-1]);
    }
    fpoints_t fr = 1;
    for (uint64_t j=PAR_md; j>=1; --j) {
      fr = fpoints_add(fpoints_mul(fr, r), f_poly[j-1]);
    }
    printf("S Q P F %u %u %u %u\n", sr, qr, pr, fr);
    printf("SQ PF %u %u\n", fpoints_mul(sr, qr), fpoints_mul(pr, fr));
    REQUIRE_DRAMATICALLY((fpoints_sub(fpoints_mul(sr, qr), fpoints_mul(pr, fr))) == 0, "SQ-PF!=0");
  }
#endif
  sdith_rng_free_xof_ctx(rng_ctx);
  memcpy(reps.H_a_seed, H_a_seed, PARAM_seed_size);
  free(random_tape);
  free(random_tape_256);
  return reps;
}

void keygen(sdith_compressed_pubkey_t* pk, sdith_compressed_key_t* sk) {
  // Generate master seed
  SDITH_randombytes(sk->m_seed, PARAM_seed_size);
...
void sub_mpc_share(mpc_share_t* x, const mpc_share_t* y) {
  ...
  for (uint64_t i = 0; i < sizeof(mpc_share_t); i++) {
    uint16_t xy = 251 + (uint16_t)x_raw[i] - (uint16_t)y_raw[i];
    x_raw[i] = p251_reduce_16(xy);
  }
}

void add_mpc_share(mpc_share_t* x, const mpc_share_t* y) {
  uint8_t* x_raw = (uint8_t*) x;
  uint8_t const* y_raw = (uint8_t const*) y;
  for (uint64_t i = 0; i < sizeof(mpc_share_t); i++) {
    uint16_t xy = (uint16_t)x_raw[i] + (uint16_t)y_raw[i];
    x_raw[i] = p251_reduce_16(xy);
  }
}
...
void expand_mpc_share_from_seed(mpc_share_t* share, const seed_t seed) {
  ...
  sdith_xof_next_bytes_mod251(rng_ctx, share, sizeof(mpc_share_t));
  for (uint64_t i_d=0; i_d < PARAM_d; ++i_d) {
...
void expand_mpc_share_from_seed4(mpc_share_t** share, seed_t *seed) {
  ...
  sdith_xof4_next_bytes_mod251(rng_ctx, (void**)share, sizeof(mpc_share_t));
...
void expand_last_mpc_share_from_seed(mpc_share_t* share, const seed_t seed) {
  ...
  XOF_CTX* rng_ctx = sdith_rng_create_xof_ctx((void*)seed, PARAM_seed_size);
  sdith_xof_next_bytes_mod251(rng_ctx, &share->a, PARAM_d * PARAM_t * 2 * sizeof(fpoints_t));
  for (uint64_t i_d=0; i_d < PARAM_d; ++i_d) {
    for (uint64_t i = 0; i < PARAM_t; ++i) {
      share->a[i_d][i] &= PAR_fpoint_mask;
      share->b[i_d][i] &= PAR_fpoint_mask;
...
//  #endif
  ...
  for (uint64_t i = 0; i < num_leafs - 1; i += 4) {
    ...
    for (uint64_t k = 0; k < 4; ++k) {
      ...
      // add it to the aux
      add_mpc_share(aux, &shares[k]);
      // add it to the main parties
      for (uint64_t j = 0; j < PARAM_D; ++j) {
        if ((((i + k) >> (PARAM_D - 1 - j)) & 1) == 0) {
          add_mpc_share(&main_party_shares[j][0], &shares[k]);
        }
  #ifndef NDEBUG
        if (((i + k) >> (PARAM_D - 1 - j)) & 1) {
          add_mpc_share(&main_party_shares[j][1], &shares[k]);
        }
  #endif
      }
    }
  ...
  // generate the last leaf share
  {
    ...
    for (uint64_t i = 0; i < PARAM_k; i++) {
      cur_share.s_A[i] = fpoints_sub(sk->s_A[i], aux->s_A[i]);
    }
    // cur_share.Q = sk.Q - aux.Q
    // cur_share.P = sk.P - aux.P
    for (uint64_t i_d = 0; i_d < PARAM_d; ++i_d) {
      for (uint64_t i = 0; i < PAR_wd; i++) {
        cur_share.q_poly[i_d][i] = fpoints_sub(sk->q_poly[i_d][i], aux->q_poly[i_d][i]);
        cur_share.p_poly[i_d][i] = fpoints_sub(sk->p_poly[i_d][i], aux->p_poly[i_d][i]);
      }
    }
    *aux = cur_share;
    ...
    // add it to the main parties
    for (uint64_t j = 0; j < PARAM_D; ++j) {
      add_mpc_share(&main_party_shares[j][1], &cur_share);
    }
#endif
    ...
    for (uint64_t k = 0; k < 4; ++k) {
      ...
      for (uint64_t j = 0; j < PARAM_D; ++j) {
        if (((((i + k) ^ hint) >> (PARAM_D - 1 - j)) & 1) == 1) {
          ...
          add_mpc_share(&main_party_shares[j][((i + k) >> (PARAM_D - 1 - j)) & 1], &shares[k]);
        }
      }
    }
  }
  ...
  if (hint != num_leafs - 1) {
    ...
    for (uint64_t j = 0; j < PARAM_D; ++j) {
      if ((((leaf ^ hint) >> (PARAM_D - 1 - j)) & 1) == 1) {
        ...
        add_mpc_share(&main_party_shares[j][(leaf >> (PARAM_D - 1 - j)) & 1], &cur_share);
      }
    }
  }
...
void generate_mpc_helper(mpc_helper_t* mpc_helper, uint32_t* r, uint32_t* eps) {
  ...
  p251_vec_mat16cols_muladd(mpc_helper->eps_f_r, f_poly, mpc_helper->compressed_eps_pow_r, PAR_md + 1);
}
...
void mpc_compute_plain_broadcasts(mpc_share_t const *share, mpc_helper_t const helper[PARAM_d],
  ...
  for (uint64_t slice=0; slice<PAR_ha_nslice; ++slice) {
    p251_vec_mat128cols_muladd_ct(s_B + 128*slice, s_A, pk->H_a[slice], PARAM_k);
  }

  for (uint64_t i_d=0; i_d < PARAM_d; ++i_d) {
    uint32_t s_r[PARAM_t] = {0};
    uint8_t compressed_s_r[16] __attribute__ ((aligned (32))) = {0};
    p251_vec_mat16cols_muladd_ct(compressed_s_r, x + i_d * PAR_md, helper[i_d].compressed_pow_r, PAR_md);
    ...
    p251_vec_mat16cols_muladd_ct(compressed_alphas, share->q_poly[i_d], helper[i_d].compressed_eps_pow_r, PAR_wd);
    helper_unpack_row(alphas[i_d], compressed_alphas);

    for (uint64_t i_t = 0; i_t < PARAM_t; i_t++) {
      alphas[i_d][i_t] = fpoints_add(alphas[i_d][i_t], share->a[i_d][i_t]);
      betas[i_d][i_t] = fpoints_add(share->b[i_d][i_t], s_r[i_t]);
    }
  }
...
void mpc_compute_communications(mpc_share_t const* share, bool with_offsets, mpc_helper_t const helper[PARAM_d],
  ...
  if (CONSTANT_TIME) {  // if on a template
    ...
    for (uint64_t slice=0; slice<PAR_ha_nslice; ++slice) {
      p251_vec_mat128cols_muladd(s_B + 128*slice, s_A, pk->H_a[slice], PARAM_k);
    }
  }
  ...
  for (uint64_t i_d=0; i_d < PARAM_d; ++i_d) {
    ...
    uint8_t compressed_s_r[16]  __attribute__ ((aligned (32))) = {0};
    if (CONSTANT_TIME) {  // if on a template
      p251_vec_mat16cols_muladd_ct(compressed_s_r, x + PAR_md * i_d, helper[i_d].compressed_pow_r, PAR_md);
    } else {
      p251_vec_mat16cols_muladd(compressed_s_r, x + PAR_md * i_d, helper[i_d].compressed_pow_r, PAR_md);
    }
    helper_unpack_row(s_r, compressed_s_r);
#ifndef NDEBUG
    for (uint64_t i_t = 0; i_t < PARAM_t; ++i_t) {
    ...
    if (CONSTANT_TIME) {
      p251_vec_mat16cols_muladd_ct(compressed_alpha, share->q_poly[i_d], helper[i_d].compressed_eps_pow_r, PAR_wd);
    } else {
      p251_vec_mat16cols_muladd(compressed_alpha, share->q_poly[i_d], helper[i_d].compressed_eps_pow_r, PAR_wd);
    }
    ...
    if (CONSTANT_TIME) {
      p251_vec_mat16cols_muladd_ct(compressed_sh_p_r, share->p_poly[i_d], helper[i_d].compressed_pow_r, PAR_wd);
    } else {
      p251_vec_mat16cols_muladd(compressed_sh_p_r, share->p_poly[i_d], helper[i_d].compressed_pow_r, PAR_wd);
    }
    helper_unpack_row(sh_p_r, compressed_sh_p_r);

    for (uint64_t i_t = 0; i_t < PARAM_t; i_t++) {
      alpha[i_d][i_t] = fpoints_add(alpha[i_d][i_t], share->a[i_d][i_t]);
      beta[i_d][i_t] = fpoints_add(share->b[i_d][i_t], s_r[i_t]);
...
void sign_offline(sdith_ctx_t* ctx, sdith_full_pubkey_t __attribute__((unused)) const* pk, sdith_full_key_t const* sk) {
  // Sample random salt
  SDITH_randombytes(ctx->salt, PARAM_salt_size);
#ifndef NDEBUG
  ...
  for (uint64_t e = 0; e < PARAM_tau; e++) {
    ...
    SDITH_randombytes(ctx->root_seeds[e], PARAM_seed_size);
#ifdef FULL_TREE
    ...
    expand_seed_binary_tree_bfs(sk, ctx->root_seeds[e], ctx->salt, ctx->msg_commit_ctx, e, &ctx->aux[e],
                                &ctx->sum_shares[e], ctx->main_party_shares[e], seeds, commits);
  }
#ifdef BENCHMARK
...
void sign_online(sdith_ctx_t* ctx, sdith_full_pubkey_t const* pk, sdith_full_key_t __attribute__((unused)) const* sk,
  ...
  sdith_xof_next_bytes_mod251(chal_prg, r, sizeof(r));
  sdith_xof_next_bytes_mod251(chal_prg, eps, sizeof(eps));
...
//#endif
  ...
  for (uint64_t e = 0; e < PARAM_tau; e++) {
    ...
    for (uint64_t i = 0; i < PARAM_D; i++) {
      ...
      for (uint64_t i_d = 0; i_d < PARAM_d; i_d++) {
        for (uint64_t i_t = 0; i_t < PARAM_t; i_t++) {
          REQUIRE_DRAMATICALLY((fpoints_add(sh_alpha[0][i_d][i_t], sh_alpha[1][i_d][i_t])) == alpha[e][i_d][i_t], "MPC alphas doesn't sum to alpha.");
          REQUIRE_DRAMATICALLY((fpoints_add(sh_beta[0][i_d][i_t], sh_beta[1][i_d][i_t])) == beta[e][i_d][i_t], "MPC betas doesn't sum to beta.");
        }
      }
      for (uint64_t i_t = 0; i_t < PARAM_t; i_t++) {
        REQUIRE_DRAMATICALLY((fpoints_add(sh_v[0][i_t], sh_v[1][i_t])) == 0, "MPC v doesn't sum to 0.");
      }
#endif
...
int SDITH_verify(sdith_full_pubkey_t const* pk, void const* msg, int msgBytes, void const* sig) {
  signature_t signature;
  ...
  for (uint64_t e = 0; e < PARAM_tau; e++) {
    ...
    for (uint64_t i = 0; i < PARAM_D; i++) {
      ...
      if (chal_party == 0) {
        for (uint64_t i_d = 0; i_d < PARAM_d; i_d++) {
          for (uint64_t i_t = 0; i_t < PARAM_t; i_t++) {
            sh_alpha[chal_party][i_d][i_t] = fpoints_sub(alpha[e][i_d][i_t], sh_alpha[1 - chal_party][i_d][i_t]);
            sh_beta[chal_party][i_d][i_t] = fpoints_sub(beta[e][i_d][i_t], sh_beta[1 - chal_party][i_d][i_t]);
          }
        }
        for (uint64_t i_t = 0; i_t < PARAM_t; i_t++) {
          sh_v[chal_party][i_t] = fpoints_sub(0, sh_v[1 - chal_party][i_t]);
        }
      }
