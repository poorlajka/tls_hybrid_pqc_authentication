...
#if defined(MIRATH_KeccakP1600times4_useAVX2)
...
{
    memset(states, 0, MIRATH_KeccakP1600times4_statesSizeInBytes);
}
...
static ALIGN(MIRATH_KeccakP1600times4_statesAlignment) const UINT64 KeccakF1600RoundConstants[24] = {
...
#define copyStateVariables(X, Y) \
 ...
 #ifdef MIRATH_KeccakP1600times4_fullUnrolling
#define FullUnrolling
#else
#define Unrolling MIRATH_KeccakP1600times4_unrolling
#endif
...
void MIRATH_KeccakP1600times4_PermuteAll_24rounds(void *states)
{
    ...
    declareABCDE
    #ifndef MIRATH_KeccakP1600times4_fullUnrolling
    unsigned int i;
    #endif
...
size_t KeccakF1600times4_FastLoop_Absorb(void *states, unsigned int laneCount, unsigned int laneOffsetParallel, unsigned int laneOffsetSerial, const unsigned char *data, size_t dataByteLen)
{
    if (laneCount == 21) {
        ...
        while(dataByteLen >= (laneOffsetParallel*3 + laneCount)*8) {
            ...
            #undef  Xor_In
            #undef  Xor_In4
            MIRATH_KeccakP1600times4_PermuteAll_24rounds(states);
...
//        unsigned int i;
        ...
        while(dataByteLen >= (laneOffsetParallel*3 + laneCount)*8) {
            MIRATH_KeccakP1600times4_AddLanesAll(states, data, laneCount, laneOffsetParallel);
            MIRATH_KeccakP1600times4_PermuteAll_24rounds(states);
...
size_t MIRATH_KeccakP1600times4_12rounds_FastLoop_Absorb(void *states, unsigned int laneCount, unsigned int laneOffsetParallel, unsigned int laneOffsetSerial, const unsigned char *data, size_t dataByteLen)
{
    if (laneCount == 21) {
        ...
        while(dataByteLen >= (laneOffsetParallel*3 + laneCount)*8) {
            ...
            #undef  Xor_In
            #undef  Xor_In4
            MIRATH_KeccakP1600times4_PermuteAll_12rounds(states);
            curData0 += laneOffsetSerial;
            curData1 += laneOffsetSerial;
            curData2 += laneOffsetSerial;
            curData3 += laneOffsetSerial;
