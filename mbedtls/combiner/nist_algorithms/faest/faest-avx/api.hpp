...
template <typename P> struct faest_scheme
{
    ...
    static int FAEST_crypto_sign_keypair(unsigned char* pk, unsigned char* sk)
    {
        if (is_owf_with_zero_sboxes(P::owf_v))
        {
            ...
            do
            {
                FAEST_rand_bytes(sk + sk_offset, FAEST_SECRET_KEY_BYTES<P> - sk_offset);
            } while (!faest_seckey<P>(sk));
            FAEST_rand_bytes(sk, sk_offset);
        ...
        else
        {
            do
            {
                FAEST_rand_bytes(sk, FAEST_SECRET_KEY_BYTES<P>);
            } while (!faest_pubkey<P>(pk, sk));
            return 0;
        }
    }

    static int FAEST_crypto_sign(unsigned char* sm, unsigned long long* smlen, const unsigned char* m,
                           unsigned long long mlen, const unsigned char* sk)
    {
        *smlen = mlen + FAEST_SIGNATURE_BYTES<P>;
        ...
        FAEST_rand_bytes(random_seed, sizeof(random_seed));
        faest_sign<P>(sm + mlen, sm, mlen, sk, random_seed, sizeof(random_seed));
        return 0;
    }

    static int FAEST_crypto_sign_open(unsigned char* m, unsigned long long* mlen, const unsigned char* sm,
