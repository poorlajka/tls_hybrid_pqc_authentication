                         ...
                         const uint8_t pt_private_key_seed[seed_length_private])
{
    ...
    for (int pi = 0; pi < m_SNOVA; ++pi)
        ...
        {
            int pj_prime = i_prime(pi, alpha);
            for (int idx = 0; idx < lsq_SNOVA; ++idx)
            {
                xQalpha1[pj_prime][alpha * lsq_SNOVA + idx] = gf16_from_nibble(Qalpha1[pi][alpha][idx]);
                xQalpha2[pj_prime][alpha * lsq_SNOVA + idx] = gf16_from_nibble(Qalpha2[pi][alpha][idx]);
                xAalpha[pj_prime][alpha * lsq_SNOVA + idx] = gf16_from_nibble(Aalpha[pi][alpha][idx]);
                xBalpha[pj_prime][alpha * lsq_SNOVA + idx] = gf16_from_nibble(Balpha[pi][alpha][idx]);
            }
        }
    ...
    do
    {
        ...
        uint32_t xLeft[m_SNOVA][alpha_SNOVA * v_SNOVA * l_SNOVA * l_SNOVA] = {0};
        uint32_t xRight[m_SNOVA][alpha_SNOVA * v_SNOVA * l_SNOVA * l_SNOVA] = {0};
        uint32_t xFvv_in_GF16Matrix[m_SNOVA][l_SNOVA][l_SNOVA] = {0};

        uint32_t xtemp_int[m_SNOVA * o_SNOVA * alpha_SNOVA * l_SNOVA * l_SNOVA] = {0};
        uint32_t xTemp[m_SNOVA][o_SNOVA][l_SNOVA][l_SNOVA][l_SNOVA][l_SNOVA] = {0};
        uint32_t xTemp_lr[m_SNOVA][o_SNOVA][alpha_SNOVA][lsq_SNOVA] = {0};
        ...
        uint32_t xTemp_Q[m_SNOVA][alpha_SNOVA][v_SNOVA][lsq_SNOVA] = {0};
            ...
            for (int alpha = 0; alpha < alpha_SNOVA; ++alpha)
                for (int jdx = 0; jdx < v_SNOVA; ++jdx)
                    for (int i1 = 0; i1 < l_SNOVA; ++i1)
                        for (int j1 = 0; j1 < l_SNOVA; ++j1)
                            for (int k1 = 0; k1 < l_SNOVA; ++k1)
                                xTemp_Q[pi][alpha][jdx][i1 * l_SNOVA + j1] ^=
                                    xVinegar_gf16[jdx][k1 * l_SNOVA + i1] * xQalpha1[pi][alpha * lsq_SNOVA + k1 * l_SNOVA + j1];
        ...
        for (int pi = 0; pi < m_SNOVA; ++pi)
            for (int alpha = 0; alpha < alpha_SNOVA; ++alpha)
                for (int jdx = 0; jdx < v_SNOVA; ++jdx)
                    for (int i1 = 0; i1 < l_SNOVA; ++i1)
                        for (int j1 = 0; j1 < l_SNOVA; ++j1)
                            xTemp_Q[pi][alpha][jdx][i1 * l_SNOVA + j1] &= 0x49249249;
            ...
            for (int alpha = 0; alpha < alpha_SNOVA; ++alpha)
                for (int jdx = 0; jdx < v_SNOVA; ++jdx)
                    for (int i1 = 0; i1 < l_SNOVA; ++i1)
                        for (int j1 = 0; j1 < l_SNOVA; ++j1)
                            for (int k1 = 0; k1 < l_SNOVA; ++k1)
                                xLeft[pi][alpha * l_SNOVA * v_SNOVA * l_SNOVA + i1 * v_SNOVA * l_SNOVA + jdx * l_SNOVA + j1] ^=
                                    xAalpha[pi][alpha * lsq_SNOVA + i1 * l_SNOVA + k1] * xTemp_Q[pi][alpha][jdx][k1 * l_SNOVA + j1];

        for (int pi = 0; pi < m_SNOVA; ++pi)
            for (int idx = 0; idx < v_SNOVA * alpha_SNOVA * lsq_SNOVA; ++idx)
                xLeft[pi][idx] = gf16_reduce(xLeft[pi][idx]);
                ...
                for (int jdx = 0; jdx < v_SNOVA; ++jdx)
                    for (int i1 = 0; i1 < l_SNOVA; ++i1)
                        for (int j1 = 0; j1 < l_SNOVA; ++j1)
                            for (int k1 = 0; k1 < l_SNOVA; ++k1)
                                xTemp_Q[pi][alpha][jdx][i1 * l_SNOVA + j1] ^=
                                    xQalpha2[pi][alpha * lsq_SNOVA + i1 * l_SNOVA + k1] * xVinegar_gf16[jdx][k1 * l_SNOVA + j1];
            ...
            for (int alpha = 0; alpha < alpha_SNOVA; ++alpha)
                for (int jdx = 0; jdx < v_SNOVA; ++jdx)
                    for (int i1 = 0; i1 < l_SNOVA; ++i1)
                        for (int j1 = 0; j1 < l_SNOVA; ++j1)
                            for (int k1 = 0; k1 < l_SNOVA; ++k1)
                                xRight[pi][alpha * l_SNOVA * v_SNOVA * l_SNOVA + j1 * v_SNOVA * l_SNOVA + jdx * l_SNOVA + i1] ^=
                                    xTemp_Q[pi][alpha][jdx][i1 * l_SNOVA + k1] * xBalpha[pi][alpha * lsq_SNOVA + k1 * l_SNOVA + j1];

        for (int pi = 0; pi < m_SNOVA; ++pi)
            for (int idx = 0; idx < v_SNOVA * alpha_SNOVA * lsq_SNOVA; ++idx)
                xRight[pi][idx] = gf16_reduce(xRight[pi][idx]);
                ...
                for (int j1 = 0; j1 < l_SNOVA; ++j1)
                {
                    ...
                    for (int alpha_i1 = 0; alpha_i1 < alpha_SNOVA * l_SNOVA; ++alpha_i1)
                        for (int jdx_k1 = 0; jdx_k1 < v_SNOVA * l_SNOVA; ++jdx_k1)
                            xtemp3[alpha_i1] ^= xLeft[mi][alpha_i1 * v_SNOVA * l_SNOVA + jdx_k1] * xF11[jdx_k1];
                    ...
                    for (int alpha = 0; alpha < alpha_SNOVA; ++alpha)
                        for (int i1 = 0; i1 < l_SNOVA; ++i1)
                            for (int j2 = 0; j2 < l_SNOVA; ++j2)
                            {
                                ...
                                xFvv_in_GF16Matrix[mj][i1][j2] ^= xtemp3[alpha * l_SNOVA + i1] *
                                                                  xRight[mi][(alpha * l_SNOVA + j2) * v_SNOVA * l_SNOVA + kdx * l_SNOVA + j1];
                            }
                }
                    ...
                    for (int alpha_i1 = 0; alpha_i1 < alpha_SNOVA * l_SNOVA; ++alpha_i1)
                        for (int jdk_k1 = 0; jdk_k1 < v_SNOVA * l_SNOVA; ++jdk_k1)
                            xtemp_int[alpha_i1 * m_SNOVA * o_SNOVA * l_SNOVA + mi_kdx_j1] ^=
                                xLeft[mi][alpha_i1 * v_SNOVA * l_SNOVA + jdk_k1] *
                                xF12[jdk_k1];
                }

        for (int idx = 0; idx < alpha_SNOVA * l_SNOVA * m_SNOVA * o_SNOVA * l_SNOVA; ++idx)
                    ...
                    for (int i1 = 0; i1 < l_SNOVA; ++i1)
                        for (int j1 = 0; j1 < l_SNOVA; ++j1)
                            for (int i2 = 0; i2 < l_SNOVA; ++i2)
                                for (int j2 = 0; j2 < l_SNOVA; ++j2)
                                    xTemp[mj][kdx][i1][j2][i2][j1] ^=
                                        xTemp_lr[mi][kdx][alpha][i1 * l_SNOVA + i2] * xBalpha[mi][alpha * lsq_SNOVA + j2 * l_SNOVA + j1];
                }
                    ...
                    for (int alpha_j1 = 0; alpha_j1 < alpha_SNOVA * l_SNOVA; ++alpha_j1)
                        for (int jdk_k1 = 0; jdk_k1 < v_SNOVA * l_SNOVA; ++jdk_k1)
                            xtemp_int[alpha_j1 * m_SNOVA * o_SNOVA * l_SNOVA + mi_kdx_i1] ^=
                                xRight[mi][alpha_j1 * v_SNOVA * l_SNOVA + jdk_k1] *
                                xF21[jdk_k1];
                }
                    ...
                    for (int i1 = 0; i1 < l_SNOVA; ++i1)
                        for (int j1 = 0; j1 < l_SNOVA; ++j1)
                            for (int i2 = 0; i2 < l_SNOVA; ++i2)
                                for (int j2 = 0; j2 < l_SNOVA; ++j2)
                                    xTemp[mj][kdx][i1][j2][i2][j1] ^=
                                        xAalpha[mi][alpha * lsq_SNOVA + i1 * l_SNOVA + j2] * xTemp_lr[mi][kdx][alpha][i2 * l_SNOVA + j1];
                }
        ...
        for (int mi2 = 0; mi2 < m_SNOVA * lsq_SNOVA; ++mi2)
        {
            ...
            for (int j2 = mi2 + 1; j2 < m_SNOVA * lsq_SNOVA; ++j2)
            {
                ...
                temp_xgf16 = SNOVA_ct_xgf16_is_not_zero(xGauss[j2][mi2]) * xGauss[j2][mi2];
                for (int k2 = mi2; k2 < m_SNOVA * lsq_SNOVA + 1; ++k2)
...
int verify_signture_opt_core(const uint8_t *pt_digest, uint64_t bytes_digest, const uint8_t *pt_signature, const public_key_expand *pkx)
{
    ...
    uint32_t xLeft[m_SNOVA][alpha_SNOVA][n_SNOVA][lsq_SNOVA] = {0};
    uint32_t xRight[m_SNOVA][alpha_SNOVA][n_SNOVA][lsq_SNOVA] = {0};
    ...
    for (int jdx = 0; jdx < n_SNOVA; ++jdx)
        for (int i1 = 0; i1 < l_SNOVA; ++i1)
            for (int j1 = 0; j1 < l_SNOVA; ++j1)
                Xsignature_in_GF16Matrix[jdx][i1 * l_SNOVA + j1] = gf16_from_nibble(signature_in_GF16Matrix[jdx][i1 * l_SNOVA + j1]);
    ...
    for (int pi = 0; pi < m_SNOVA; ++pi)
        for (int alpha = 0; alpha < alpha_SNOVA; ++alpha)
        {
            ...
            int pi_prime = i_prime_inv(pi, alpha);
            for (int idx = 0; idx < lsq_SNOVA; ++idx)
            {
                xAalpha[idx] = gf16_from_nibble(pkx->map1.Aalpha[pi_prime][alpha][idx]);
                xBalpha[idx] = gf16_from_nibble(pkx->map1.Balpha[pi_prime][alpha][idx]);
                xQalpha1[idx] = gf16_from_nibble(pkx->map1.Qalpha1[pi_prime][alpha][idx]);
                xQalpha2[idx] = gf16_from_nibble(pkx->map1.Qalpha2[pi_prime][alpha][idx]);
            }
            ...
            for (int jdx = 0; jdx < n_SNOVA; ++jdx)
                for (int i1 = 0; i1 < l_SNOVA; ++i1)
                    for (int j1 = 0; j1 < l_SNOVA; ++j1)
                        for (int k1 = 0; k1 < l_SNOVA; ++k1)
                            xLeft[pi][alpha][jdx][i1 * l_SNOVA + j1] ^=
                                xAalpha[i1 * l_SNOVA + k1] * xTemp_Q[jdx][k1 * l_SNOVA + j1];

            for (int jdx = 0; jdx < n_SNOVA; ++jdx)
                for (int idx = 0; idx < lsq_SNOVA; ++idx)
                    xLeft[pi][alpha][jdx][idx] = gf16_reduce(xLeft[pi][alpha][jdx][idx]);
            ...
            for (int jdx = 0; jdx < n_SNOVA; ++jdx)
                for (int i1 = 0; i1 < l_SNOVA; ++i1)
                    for (int j1 = 0; j1 < l_SNOVA; ++j1)
                        for (int k1 = 0; k1 < l_SNOVA; ++k1)
                            xRight[pi][alpha][jdx][i1 * l_SNOVA + j1] ^=
                                xTemp_Q[jdx][i1 * l_SNOVA + k1] * xBalpha[k1 * l_SNOVA + j1];

            for (int jdx = 0; jdx < n_SNOVA; ++jdx)
                for (int idx = 0; idx < lsq_SNOVA; ++idx)
                    xRight[pi][alpha][jdx][idx] = gf16_reduce(xRight[pi][alpha][jdx][idx]);
        }
                    ...
                    for (int alpha = 0; alpha < alpha_SNOVA; ++alpha)
                        for (int k1 = 0; k1 < l_SNOVA; ++k1)
                            for (int dk = 0; dk < n_SNOVA; ++dk)
                                Intermediate[alpha][i1 * l_SNOVA + j1][dk] ^= xLeft[mi][alpha][dj][i1 * l_SNOVA + k1] *
        ...
        for (int alpha = 0; alpha < alpha_SNOVA; ++alpha)
        {
            ...
            for (int i1 = 0; i1 < l_SNOVA; ++i1)
                for (int j1 = 0; j1 < l_SNOVA; ++j1)
                    for (int k1 = 0; k1 < l_SNOVA; ++k1)
                        for (int dk = 0; dk < n_SNOVA; ++dk)
                            res[mj][i1 * l_SNOVA + j1] ^= Intermediate[alpha][i1 * l_SNOVA + k1][dk] *
                                                          xRight[mi][alpha][dk][k1 * l_SNOVA + j1];
        }
    }
#else
    uint32_t res[m_SNOVA][lsq_SNOVA] = {0};
    uint32_t xPublic[m_SNOVA][n_SNOVA][lsq_SNOVA][n_SNOVA];
    ...
    for (int mi = 0; mi < m_SNOVA; ++mi)
        for (int dj = 0; dj < n_SNOVA; ++dj)
        {
            ...
            if (dj < v_SNOVA)
            {
                for (int dk = 0; dk < v_SNOVA; dk++)
                    for (size_t idx = 0; idx < lsq_SNOVA; idx++)
                        xPublic[mi][dj][idx][dk] = gf16_from_nibble(pkx->map1.P11[mi][dj][dk][idx]);

                for (int dk = v_SNOVA; dk < n_SNOVA; dk++)
                    for (size_t idx = 0; idx < lsq_SNOVA; idx++)
                        xPublic[mi][dj][idx][dk] = gf16_from_nibble(pkx->map1.P12[mi][dj][dk - v_SNOVA][idx]);
            }
            else
            {
                for (int dk = 0; dk < v_SNOVA; dk++)
                    for (size_t idx = 0; idx < lsq_SNOVA; idx++)
                        xPublic[mi][dj][idx][dk] = gf16_from_nibble(pkx->map1.P21[mi][dj - v_SNOVA][dk][idx]);

                for (int dk = v_SNOVA; dk < n_SNOVA; dk++)
                    for (size_t idx = 0; idx < lsq_SNOVA; idx++)
                        xPublic[mi][dj][idx][dk] = gf16_from_nibble(pkx->P22[mi][dj - v_SNOVA][dk - v_SNOVA][idx]);
            }
        }
                        ...
                        for (int k1 = 0; k1 < l_SNOVA; ++k1)
                            for (int j1 = 0; j1 < l_SNOVA; ++j1)
                                Intermediate[dk * alpha_SNOVA * lsq_SNOVA + alpha * lsq_SNOVA + i1 * l_SNOVA + j1] ^=
                                    xLeft[mi][alpha][dj][i1 * l_SNOVA + k1] *
                                    xPublic[mi][dj][k1 * l_SNOVA + j1][dk];
                    ...
                    for (int k1 = 0; k1 < l_SNOVA; ++k1)
                        for (int dk = 0; dk < n_SNOVA; ++dk)
                            res[mj][i1 * l_SNOVA + k1] ^=
                                Intermediate[dk * alpha_SNOVA * lsq_SNOVA + alpha * lsq_SNOVA + i1 * l_SNOVA + j1] *
                                xRight[mi][alpha][dk][j1 * l_SNOVA + k1];
        }
    }
#endif
