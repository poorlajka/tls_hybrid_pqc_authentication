 ...
 */
void SNOVA_shake256(const uint8_t* pt_seed_array, int input_bytes, uint8_t* pt_output_array, int output_bytes) {
...
void gen_a_FqS_ct(gf16_t* c, gf16m_t pt_matrix) {
    ...
    uint8_t zero = SNOVA_ct_gf16_is_not_zero(c[rank - 1]);
    uint8_t val = zero * c[rank - 1] + (1 - zero) * (15 + SNOVA_ct_gf16_is_not_zero(c[0]) - c[0]);
...
void gen_seeds_and_T12(T12_t T12, const uint8_t* seed) {
    ...
    SNOVA_shake256(seed, seed_length_private, prng_output_private, bytes_prng_private);
 ...
 * @param pk - input
 */
void SNOVA_expand_public_pack_core(uint8_t* pkx_pck, const uint8_t *pk) {
