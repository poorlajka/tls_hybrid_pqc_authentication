...
void gen_ABQ(const char *abq_seed)
{
    ...
    SNOVA_shake256((uint8_t *)abq_seed, strlen(abq_seed),
             rng_out, m_SNOVA * alpha_SNOVA * (lsq_SNOVA + l_SNOVA));
 ...
 */
void SNOVA_generate_keys_ssk(uint8_t* pk, uint8_t* ssk, const uint8_t* pkseed, const uint8_t* skseed) {
    snova_init();
 ...
 * @param skseed - pointer to input private key seed.
 */
void SNOVA_generate_keys_esk(uint8_t* pk, uint8_t* esk, const uint8_t* pkseed, const uint8_t* skseed) {
    snova_init();
    snova_key_elems key_elems;
    generate_keys_core(&key_elems, pkseed, skseed);
    pk_pack(pk, &key_elems);
 ...
 * @param ssk - pointer to output private key.
 */
void SNOVA_generate_pk_with_ssk(uint8_t* pk, const uint8_t* ssk) {
    snova_init();
 ...
 */
void SNOVA_generate_pk_with_esk(uint8_t* pk, const uint8_t* esk) {
    snova_init();
    snova_key_elems key_elems;
    sk_gf16 sk_upk;
 ...
 */
void SNOVA_sign_digest_ssk(uint8_t* pt_signature, const uint8_t* digest, uint64_t bytes_digest, uint8_t* array_salt,
                     const uint8_t* ssk) {
    snova_init();
    snova_key_elems key_elems;
 ...
 */
void SNOVA_sign_digest_esk(uint8_t* pt_signature, const uint8_t* digest, uint64_t bytes_digest, uint8_t* array_salt,
                     const uint8_t* esk) {
    snova_init();
    sk_gf16 sk_upk;
    sk_unpack(&sk_upk, esk);
 ...
 */
int SNOVA_verify_signture(const uint8_t* pt_digest, uint64_t bytes_digest, const uint8_t* pt_signature, const uint8_t* pk) {
    snova_init();
 ...
 * @returns - 0 if signature could be verified correctly and -1 otherwise
 */
int SNOVA_verify_signture_pkx(const uint8_t* pt_digest, uint64_t bytes_digest, const uint8_t* pt_signature, const uint8_t* pkx) {
    snova_init();
    return verify_pkx_core(pt_digest, bytes_digest, pt_signature, pkx);
}

void SNOVA_expand_public_pack(uint8_t *pkx, const uint8_t *pk) {
    snova_init();
    SNOVA_expand_public_pack_core(pkx, pk);
}