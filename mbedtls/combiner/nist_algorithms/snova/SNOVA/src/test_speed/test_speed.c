...
int main() {
    ...
    for (int i = 0; i < TEST_SPEED_N; i++) {
        ...
        t0[i * 2] = get_cycles();
        SNOVA_generate_keys_ssk(pk, ssk, pt_public_key_seed, pt_private_key_seed);
        t0[i * 2 + 1] = get_cycles();

        t1[i * 2] = get_cycles();
        SNOVA_generate_keys_esk(pk, esk, pt_public_key_seed, pt_private_key_seed);
        t1[i * 2 + 1] = get_cycles();

        t2[i * 2] = get_cycles();
        SNOVA_sign_digest_ssk(array_signature2, array_digest, 64, array_salt, ssk);
        t2[i * 2 + 1] = get_cycles();

        t3[i * 2] = get_cycles();
        SNOVA_sign_digest_esk(array_signature1, array_digest, 64, array_salt, esk);
        t3[i * 2 + 1] = get_cycles();

        t4[i * 2] = get_cycles();
        r0 += SNOVA_verify_signture(array_digest, 64, array_signature2, pk);
        t4[i * 2 + 1] = get_cycles();
        ...
        t6[i * 2] = get_cycles();
        r1 += SNOVA_verify_signture_pkx(array_digest, 64, array_signature2, pkx);
        t6[i * 2 + 1] = get_cycles();
    }

    printf("SNOVA_generate_keys_ssk: \n");
    analysis(t0, TEST_SPEED_N, CPU_CLOCK);
    printf("------------------------------------------------\n");
    printf("SNOVA_generate_keys_esk: \n");
    analysis(t1, TEST_SPEED_N, CPU_CLOCK);
    printf("------------------------------------------------\n");
    printf("SNOVA_sign_digest_ssk: \n");
    analysis(t2, TEST_SPEED_N, CPU_CLOCK);
    printf("------------------------------------------------\n");
    printf("SNOVA_sign_digest_esk: \n");
    analysis(t3, TEST_SPEED_N, CPU_CLOCK);
    printf("------------------------------------------------\n");
    printf("verify_signture: \n");
    analysis(t4, TEST_SPEED_N, CPU_CLOCK);
    printf("------------------------------------------------\n");
    printf("SNOVA_expand_public_pack: \n");
