...
void SNOVA_oqs_aes128_load_schedule_ni(const uint8_t *key, void **_schedule) {
...
void SNOVA_oqs_aes128_free_schedule_ni(void *schedule) {
    if (schedule != NULL) {
...
int AES_128_CTR_NI(unsigned char *output, size_t outputByteLen,
    ...
    void *schedule = NULL;
    SNOVA_oqs_aes128_load_schedule_ni(input, &schedule);
    oqs_aes128_ctr_enc_sch_ni(schedule, output, outputByteLen);
    SNOVA_oqs_aes128_free_schedule_ni(schedule);
