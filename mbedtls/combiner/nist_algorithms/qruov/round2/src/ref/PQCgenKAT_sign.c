...
main()
{
    ...
    QRUOV_randombytes_init(entropy_input, NULL, 256);
    for (int i=0; i<100; i++) {
        fprintf(fp_req, "count = %d\n", i);
        QRUOV_randombytes(seed, 48);
        fprintBstr(fp_req, "seed = ", seed, 48);
        mlen = 33*(i+1);
        fprintf(fp_req, "mlen = %llu\n", mlen);
        QRUOV_randombytes(msg, mlen);
    ...
    do {
        ...
        QRUOV_randombytes_init(seed, NULL, 256);
        ...
        if ( (ret_val = QRUOV_crypto_sign_keypair(pk, sk)) != 0) {
        ...
        if ( (ret_val = QRUOV_crypto_sign(sm, &smlen, m, mlen, sk)) != 0) {
        ...
        if ( (ret_val = QRUOV_crypto_sign_open(m1, &mlen1, sm, smlen, pk)) != 0) {
