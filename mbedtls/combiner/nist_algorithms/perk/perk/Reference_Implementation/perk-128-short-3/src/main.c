...
static void init_randomness(void) {
    ...
    if (0 != getentropy(seed, sizeof(seed))) {
        printf("failed to get entropy for PERK_randombytes()\n");
        exit(1);
    }
#endif
...
int main(void) {
    ...
    PERK_crypto_sign_keypair(pk, sk);
    PERK_crypto_sign(sm, &smlen, m, sizeof(m), sk);
    ret = PERK_crypto_sign_open(m, &mlen, sm, smlen, pk);
    if (ret == 0) {
        printf("\nDone");
    } else {
        printf("\nFailed");
