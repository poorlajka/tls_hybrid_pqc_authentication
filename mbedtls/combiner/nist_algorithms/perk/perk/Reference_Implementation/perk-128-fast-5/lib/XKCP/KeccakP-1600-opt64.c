...
#if defined(PERK_KeccakP1600_useLaneComplementing)
#define UseBebigokimisa
#endif

#if defined(_MSC_VER)
#define ROL64(a, offset) _rotl64(a, offset)
#elif defined(PERK_KeccakP1600_useSHLD)
    #define ROL64(x,N) ({ \
...
#ifdef PERK_KeccakP1600_fullUnrolling
#define FullUnrolling
#else
#define Unrolling PERK_KeccakP1600_unrolling
#endif
#include "KeccakP-1600-unrolling.macros"
#include "SnP-Relaned.h"

static const uint64_t PERK_KeccakF1600RoundConstants[24] = {
...
{
    memset(state, 0, 200);
#ifdef PERK_KeccakP1600_useLaneComplementing
...
void PERK_KeccakP1600_AddBytesInLane(void *state, unsigned int lanePosition, const unsigned char *data, unsigned int offset, unsigned int length)
{
...
void PERK_KeccakP1600_AddLanes(void *state, const unsigned char *data, unsigned int laneCount)
{
#if (PLATFORM_BYTE_ORDER == IS_LITTLE_ENDIAN)
    unsigned int i = 0;
...
#if (PLATFORM_BYTE_ORDER != IS_LITTLE_ENDIAN)
void PERK_KeccakP1600_AddByte(void *state, unsigned char byte, unsigned int offset)
{
    uint64_t lane = byte;
    lane <<= (offset%8)*8;
...
{
    SnP_AddBytes(state, data, offset, length, PERK_KeccakP1600_AddLanes, PERK_KeccakP1600_AddBytesInLane, 8);
}

/* ---------------------------------------------------------------- */

void PERK_KeccakP1600_OverwriteBytesInLane(void *state, unsigned int lanePosition, const unsigned char *data, unsigned int offset, unsigned int length)
{
#if (PLATFORM_BYTE_ORDER == IS_LITTLE_ENDIAN)
#ifdef PERK_KeccakP1600_useLaneComplementing
...
void PERK_KeccakP1600_OverwriteLanes(void *state, const unsigned char *data, unsigned int laneCount)
{
#if (PLATFORM_BYTE_ORDER == IS_LITTLE_ENDIAN)
#ifdef PERK_KeccakP1600_useLaneComplementing
...
{
    SnP_OverwriteBytes(state, data, offset, length, PERK_KeccakP1600_OverwriteLanes, PERK_KeccakP1600_OverwriteBytesInLane, 8);
}
...
void PERK_KeccakP1600_Permute_Nrounds(void *state, unsigned int nr)
{
    declareABCDE
    unsigned int i;
...
{
    declareABCDE
    #ifndef PERK_KeccakP1600_fullUnrolling
    unsigned int i;
    #endif
    uint64_t *stateAsLanes = (uint64_t*)state;
...
void PERK_KeccakP1600_ExtractBytesInLane(const void *state, unsigned int lanePosition, unsigned char *data, unsigned int offset, unsigned int length)
{
    uint64_t lane = ((uint64_t*)state)[lanePosition];
#ifdef PERK_KeccakP1600_useLaneComplementing
    if ((lanePosition == 1) || (lanePosition == 2) || (lanePosition == 8) || (lanePosition == 12) || (lanePosition == 17) || (lanePosition == 20))
...
void PERK_KeccakP1600_ExtractLanes(const void *state, unsigned char *data, unsigned int laneCount)
{
#if (PLATFORM_BYTE_ORDER == IS_LITTLE_ENDIAN)
...
{
    SnP_ExtractBytes(state, data, offset, length, PERK_KeccakP1600_ExtractLanes, PERK_KeccakP1600_ExtractBytesInLane, 8);
}

/* ---------------------------------------------------------------- */

void PERK_KeccakP1600_ExtractAndAddBytesInLane(const void *state, unsigned int lanePosition, const unsigned char *input, unsigned char *output, unsigned int offset, unsigned int length)
{
    uint64_t lane = ((uint64_t*)state)[lanePosition];
#ifdef PERK_KeccakP1600_useLaneComplementing
    if ((lanePosition == 1) || (lanePosition == 2) || (lanePosition == 8) || (lanePosition == 12) || (lanePosition == 17) || (lanePosition == 20))
...
void PERK_KeccakP1600_ExtractAndAddLanes(const void *state, const unsigned char *input, unsigned char *output, unsigned int laneCount)
{
    unsigned int i;
#if (PLATFORM_BYTE_ORDER != IS_LITTLE_ENDIAN)
...
{
    SnP_ExtractAndAddBytes(state, input, output, offset, length, PERK_KeccakP1600_ExtractAndAddLanes, PERK_KeccakP1600_ExtractAndAddBytesInLane, 8);
}
