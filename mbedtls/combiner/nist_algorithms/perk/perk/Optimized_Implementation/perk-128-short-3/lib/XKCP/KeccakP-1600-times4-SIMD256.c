...
#if defined(PERK_KeccakP1600times4_useAVX2)
...
static ALIGN(PERK_KeccakP1600times4_statesAlignment) const uint64_t rho8[4] = {0x0605040302010007, 0x0E0D0C0B0A09080F, 0x1615141312111017, 0x1E1D1C1B1A19181F};
static ALIGN(PERK_KeccakP1600times4_statesAlignment) const uint64_t rho56[4] = {0x0007060504030201, 0x080F0E0D0C0B0A09, 0x1017161514131211, 0x181F1E1D1C1B1A19};
...
\
    ...
    XOReq256(E##ba, CONST256_64(PERK_KeccakF1600RoundConstants[i])); \
    Ca = E##ba; \
    XOReq256(A##mo, Do); \
...
static ALIGN(PERK_KeccakP1600times4_statesAlignment) const uint64_t PERK_KeccakF1600RoundConstants[24] = {
...
#define copyStateVariables(X, Y) \
 ...
 #ifdef PERK_KeccakP1600times4_fullUnrolling
#define FullUnrolling
#else
#define Unrolling PERK_KeccakP1600times4_unrolling
#endif
...
void PERK_KeccakP1600times4_PermuteAll_24rounds(void *states)
{
    ...
    declareABCDE
    #ifndef PERK_KeccakP1600times4_fullUnrolling
    unsigned int i;
    #endif
...
size_t PERK_KeccakF1600times4_FastLoop_Absorb(void *states, unsigned int laneCount, unsigned int laneOffsetParallel, unsigned int laneOffsetSerial, const unsigned char *data, size_t dataByteLen)
{
    if (laneCount == 21) {
#if 0
        ...
        while(dataByteLen >= (laneOffsetParallel*3 + laneCount)*8) {
            ...
            #undef  Xor_In
            #undef  Xor_In4
            PERK_KeccakP1600times4_PermuteAll_24rounds(states);
        ...
        while(dataByteLen >= (laneOffsetParallel*3 + laneCount)*8) {
            PERK_KeccakP1600times4_AddLanesAll(states, data, laneCount, laneOffsetParallel);
            PERK_KeccakP1600times4_PermuteAll_24rounds(states);
...
size_t PERK_KeccakP1600times4_12rounds_FastLoop_Absorb(void *states, unsigned int laneCount, unsigned int laneOffsetParallel, unsigned int laneOffsetSerial, const unsigned char *data, size_t dataByteLen)
{
    if (laneCount == 21) {
#if 0
        const unsigned char *dataStart = data;
        ...
        while(dataByteLen >= (laneOffsetParallel*3 + laneCount)*8) {
            ...
            #undef  Xor_In
            #undef  Xor_In4
            PERK_KeccakP1600times4_PermuteAll_12rounds(states);
            curData0 += laneOffsetSerial;
            curData1 += laneOffsetSerial;
            curData2 += laneOffsetSerial;
            curData3 += laneOffsetSerial;
...
size_t PERK_KeccakP1600times4_KravatteCompress(uint64_t *xAccu, uint64_t *kRoll, const unsigned char *input, size_t inputByteLen)
{
    uint64_t *in64 = (uint64_t *)input;
    size_t    nBlocks = inputByteLen / (4 * 200);
    declareABCDE
    #if    !defined(PERK_KeccakP1600times4_fullUnrolling)
    unsigned int i;
    #endif
...
size_t PERK_KeccakP1600times4_KravatteExpand(uint64_t *yAccu, const uint64_t *kRoll, unsigned char *output, size_t outputByteLen)
{
