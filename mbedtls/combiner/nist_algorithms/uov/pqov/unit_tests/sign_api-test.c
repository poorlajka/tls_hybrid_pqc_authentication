...
int main(void) {
    ...
    printf("===========  test UOV_crypto_sign_keypair(), UOV_crypto_sign(), and UOV_crypto_sign_open()  ================\n\n");
    for (unsigned i = 0; i < TEST_RUN; i++) {
        if ( i < TEST_GENKEY ) {
            int r0;
            r0 = UOV_crypto_sign_keypair( pk, sk);
            if ( 0 != r0 ) {
    ...
    printf("===========  test UOV_crypto_sign_keypair(), crypto_sign_signature(), and crypto_sign_verify()  ================\n\n");
    ...
    for (unsigned i = 0; i < TEST_RUN; i++) {
        int rc;
        rc = UOV_crypto_sign_keypair( pk, sk);
        if ( 0 != rc ) {
