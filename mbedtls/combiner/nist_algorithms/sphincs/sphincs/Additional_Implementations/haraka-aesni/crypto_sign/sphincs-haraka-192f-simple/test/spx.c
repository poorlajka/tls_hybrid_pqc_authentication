...
int main()
{
    ...
    if (SPHINCS_crypto_sign_keypair(pk, sk)) {
        printf("failed!\n");
        return -1;
    }
    printf("successful.\n");
    ...
    for (i = 0; i < SPX_SIGNATURES; i++) {
        ...
        SPHINCS_crypto_sign(sm, &smlen, m, SPX_MLEN, sk);
        ...
        if (SPHINCS_crypto_sign_open(mout, &mlen, sm, smlen, pk)) {
        ...
        if (SPHINCS_crypto_sign_open(sm, &mlen, sm, smlen, pk)) {
