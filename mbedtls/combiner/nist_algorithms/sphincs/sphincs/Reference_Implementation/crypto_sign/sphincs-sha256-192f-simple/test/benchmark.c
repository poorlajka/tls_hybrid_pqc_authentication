...
int main()
{
    ...
    SPHINCS_randombytes(m, SPX_MLEN);
    SPHINCS_randombytes(addr, SPX_ADDR_BYTES);
    ...
    MEASURE("Generating keypair.. ", 1, SPHINCS_crypto_sign_keypair(pk, sk));
    MEASURE("  - WOTS pk gen..    ", (1 << SPX_TREE_HEIGHT), wots_gen_pk(wots_pk, sk, pk, (uint32_t *) addr));
    MEASURE("Signing..            ", 1, SPHINCS_crypto_sign(sm, &smlen, m, SPX_MLEN, sk));
    ...
    MEASURE("Verifying..          ", 1, SPHINCS_crypto_sign_open(mout, &mlen, sm, smlen, pk));
