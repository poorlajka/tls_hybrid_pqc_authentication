...
/* Provided externally. */
void HAWK_randombytes(unsigned char *x, unsigned long long xlen);

/*
 * Wrapper for HAWK_randombytes() that follows the hawk_rng type.
 */
static void
hrng(void *ctx, void *dst, size_t len)
{
	(void)ctx;
	HAWK_randombytes(dst, len);
}

/* see api.h */
int
HAWK_crypto_sign_keypair(unsigned char *pk, unsigned char *sk)
{
...
/* see api.h */
int
HAWK_crypto_sign(unsigned char *sm, unsigned long long *smlen,
...
/* see api.h */
int
HAWK_crypto_sign_open(unsigned char *m, unsigned long long *mlen,
