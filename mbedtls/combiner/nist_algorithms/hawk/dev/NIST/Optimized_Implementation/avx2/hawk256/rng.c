/*
 * 2023-04-04:
 * This file has been modified to include a perfunctory AES-256/ECB
 * implementation (not constant-time, for tests only) instead of using
 * OpenSSL. This allows this code to be compiled on systems that do not
 * have OpenSSL installed, typically Windows machines with MSVC.
 *
 * Original copyright notice (from NIST) follows below.
 */
...
AES256_CTR_DRBG_struct  HAWK_DRBG_ctx;

void    HAWK_AES256_ECB(unsigned char *key, unsigned char *ctr, unsigned char *buffer);

/*
 HAWK_seedexpander_init()
...
HAWK_seedexpander(AES_XOF_struct *ctx, unsigned char *x, unsigned long xlen)
{
    ...
    while ( xlen > 0 ) {
        ...
        HAWK_AES256_ECB(ctx->key, ctx->ctr, ctx->buffer);
        ctx->buffer_pos = 0;
...
void
HAWK_AES256_ECB(unsigned char *key, unsigned char *ctr, unsigned char *buffer)
{
    EVP_CIPHER_CTX *ctx;
                 ...
                 int security_strength)
{
    ...
    memset(HAWK_DRBG_ctx.Key, 0x00, 32);
    memset(HAWK_DRBG_ctx.V, 0x00, 16);
    HAWK_AES256_CTR_DRBG_Update(seed_material, HAWK_DRBG_ctx.Key, HAWK_DRBG_ctx.V);
    HAWK_DRBG_ctx.reseed_counter = 1;
}
...
HAWK_randombytes(unsigned char *x, unsigned long long xlen)
{
    ...
    while ( xlen > 0 ) {
        //increment V
        for (int j=15; j>=0; j--) {
            if ( HAWK_DRBG_ctx.V[j] == 0xff )
                HAWK_DRBG_ctx.V[j] = 0x00;
            else {
                HAWK_DRBG_ctx.V[j]++;
                break;
            }
        }
        HAWK_AES256_ECB(HAWK_DRBG_ctx.Key, HAWK_DRBG_ctx.V, block);
        if ( xlen > 15 ) {
            memcpy(x+i, block, 16);
            i += 16;
            xlen -= 16;
            ...
            // cast to size_t to avoid an MSVC warning in 32-bit mode
            memcpy(x+i, block, (size_t)xlen);
            xlen = 0;
        }
    }
    HAWK_AES256_CTR_DRBG_Update(NULL, HAWK_DRBG_ctx.Key, HAWK_DRBG_ctx.V);
    HAWK_DRBG_ctx.reseed_counter++;
}
                       ...
                       unsigned char *V)
{
    ...
    for (int i=0; i<3; i++) {
        ...
        HAWK_AES256_ECB(Key, V, temp+16*i);
    }
