 ...
 * of making the FALCON_crypto_sign_keypair(), FALCON_crypto_sign(), and
...
int
FALCON_crypto_sign_keypair(unsigned char *pk, unsigned char *sk)
{
	TEMPALLOC union {
...
int
FALCON_crypto_sign(unsigned char *sm, unsigned long long *smlen,
...
int
FALCON_crypto_sign_open(unsigned char *m, unsigned long long *mlen,
