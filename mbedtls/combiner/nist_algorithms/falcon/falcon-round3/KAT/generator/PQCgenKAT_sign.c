...
main()
{
    ...
    do {
        ...
        if ( (ret_val = FALCON_crypto_sign_keypair(pk, sk)) != 0) {
            printf("FALCON_crypto_sign_keypair returned <%d>\n", ret_val);
            return KAT_CRYPTO_FAILURE;
        }
        fprintBstr(fp_rsp, "pk = ", pk, CRYPTO_PUBLICKEYBYTES);
        fprintBstr(fp_rsp, "sk = ", sk, CRYPTO_SECRETKEYBYTES);
        
        if ( (ret_val = FALCON_crypto_sign(sm, &smlen, m, mlen, sk)) != 0) {
            printf("FALCON_crypto_sign returned <%d>\n", ret_val);
            return KAT_CRYPTO_FAILURE;
        }
        ...
        if ( (ret_val = FALCON_crypto_sign_open(m1, &mlen1, sm, smlen, pk)) != 0) {
            printf("FALCON_crypto_sign_open returned <%d>\n", ret_val);
            return KAT_CRYPTO_FAILURE;
        }
        
        if ( mlen != mlen1 ) {
            printf("FALCON_crypto_sign_open returned bad 'mlen': Got <%llu>, expected <%llu>\n", mlen1, mlen);
            return KAT_CRYPTO_FAILURE;
        }
        
        if ( memcmp(m, m1, mlen) ) {
            printf("FALCON_crypto_sign_open returned bad 'm' value\n");
            return KAT_CRYPTO_FAILURE;
        }
