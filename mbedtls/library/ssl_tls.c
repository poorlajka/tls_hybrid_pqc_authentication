...
static const uint16_t ssl_preset_default_sig_algs[] = {
    MBEDTLS_TLS1_3_SIG_PQC_HYBRID_SHA256,
...
{
    if (mbedtls_pk_can_do(pk, MBEDTLS_PK_PQC_HYBRID)) {
        return MBEDTLS_SSL_SIG_PQC_HYBRID;
    }
#if defined(MBEDTLS_RSA_C)
...
{
    switch (sig) {
        case MBEDTLS_SSL_SIG_PQC_HYBRID:
            return MBEDTLS_PK_PQC_HYBRID;
#if defined(MBEDTLS_RSA_C)
