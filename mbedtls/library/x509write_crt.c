                                                    ...
                                                    unsigned char tag)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    // BUFFER SIZE HERE
    unsigned char buf[MBEDTLS_MPI_MAX_SIZE * 2 + 20 + 400000]; /* tag, length + 2xMPI */
    unsigned char *c = buf + sizeof(buf);
    size_t len = 0;
#if defined(MBEDTLS_USE_PSA_CRYPTO)
                              ...
                              void *p_rng)
{
    ...
    if (mbedtls_pk_can_do(ctx->issuer_key, MBEDTLS_PK_PQC_HYBRID)) {
        pk_alg = MBEDTLS_PK_PQC_HYBRID;
    }
