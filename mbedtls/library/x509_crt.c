                                  ...
                                  const mbedtls_pk_context *pk)
{
    ...
    if (pk_alg == MBEDTLS_PK_PQC_HYBRID) {
        // TODO THIS SHOULD CHECK THE VALIDITY OF THE PQC HYBRID KEY
        return 0;
    }
    ...
    if (crt->ext_types & MBEDTLS_X509_EXT_SUBJECT_ALT_NAME) {
        ...
        for (name = &crt->subject; name != NULL; name = name->next) {
            if (MBEDTLS_OID_CMP(MBEDTLS_OID_AT_CN, &name->oid) == 0) { //&&
                //x509_crt_check_cn(&name->val, cn, cn_len) == 0) { //TODO: Ugly hack to make it work fix if I have time
                return;
            }
        }
