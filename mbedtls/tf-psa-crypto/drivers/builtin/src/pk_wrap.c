...
int mbedtls_pqc_hybrid_parse_pubkey (mbedtls_pk_context *pk, 
                                const unsigned char *key, size_t keylen) {
    hybrid_t* hybrid = malloc(sizeof(hybrid_t));
    pk->pk_ctx = hybrid;
    int ret = combiner_parse_pubkey(hybrid, key, keylen);
    return ret;
}

int mbedtls_pqc_hybrid_write_pubkey (unsigned char **p, unsigned char *start,
                            const mbedtls_pk_context *pk) {
    uint8_t buf[400000];
    const mbedtls_pqc_hybrid_context* hybrid = (const mbedtls_pqc_hybrid_context*) pk->pk_ctx;

    int ret = combiner_write_pubkey(hybrid, buf, sizeof(buf));
    if (ret == -1) {
...
/*
    Memory leak schmemory leak :)
*/
static void pqc_hybrid_free (void* ctx) {
    const mbedtls_pqc_hybrid_context* hybrid = (const mbedtls_pqc_hybrid_context*) ctx;
    printf("pk size: %d", (int)hybrid->keypair.public_key_len);
    //mbedtls_free(ctx);
}
...
static int pqc_hybrid_sign (mbedtls_pk_context *pk, mbedtls_md_type_t md_alg,
    ...
    *sig_len = 0;
    combiner_write_signature(hybrid, sig, sig_len);

    printf("\nlength of signature: %zu\n", *sig_len);
    return ret;
}
...
static int pqc_hybrid_verify (mbedtls_pk_context *pk, mbedtls_md_type_t md_alg,
    ...
    mbedtls_pqc_hybrid_context* hybrid = (mbedtls_pqc_hybrid_context*) pk->pk_ctx;
    combiner_parse_signature(hybrid, 
        (msg_t) {.content = sig, .len = sig_len}, 
        (msg_t) {.content = hash, .len = hash_len});

    int verify_passed = combiner_verify(*hybrid, 
        (msg_t) {.content = hash, .len = hash_len});
